...
use crate::ir::{
    Junction, Label, Mirror, Net, Paper, Pin, PinInstance, PinType,
    Property, RenderHint, Schematic, Sheet, SheetPin, SheetProperty, Stroke, StrokeType, Symbol,
    SymbolInstance, TextEffects, TitleBlock, Wire,
};
...
fn value_to_schematic(value: &Value) -> Result<Schematic> {
    ...
    if let Some(nets) = obj.get("nets").and_then(|v| v.as_array()) {
        ...
        eprintln!("[json5_parser] WARNING: 'nets' key not found or not an array. Top-level keys: {:?}",
