...
fn parse_point_list(v: &Value, key: &str) -> Vec<(f64, f64)> {
    v.get(key)
        .and_then(|v| v.as_array())
        .map(|arr| arr.iter().filter_map(|i| parse_point(i)).collect())
        .unwrap_or_default()
}
...
fn value_to_board(value: &Value) -> Result<Board> {
    ...
    board.generator = get_str(obj, "generator").unwrap_or("kicad-json5").to_string();
        ...
        for sv in segs {
            if let Some(so) = sv.as_object() {
                board.segments.push(Segment {
                    start: parse_point(sv.get("start").unwrap_or(&Value::Null)).unwrap_or((0.0, 0.0)),
...
fn value_to_footprint(v: &Value) -> Footprint {
    ...
    fp.attr = match get_str(obj, "attr") {
        ...
        Some(_) => {
            let vals: Vec<String> = v.get("attr")
        ...
        for lv in lines {
            if let Some(lo) = lv.as_object() {
                fp.fp_lines.push(FpLine {
                    start: parse_point(lv.get("start").unwrap_or(&Value::Null)).unwrap_or((0.0, 0.0)),
        ...
        for cv in circles {
            if let Some(co) = cv.as_object() {
                fp.fp_circles.push(FpCircle {
                    center: parse_point(cv.get("center").unwrap_or(&Value::Null)).unwrap_or((0.0, 0.0)),
        ...
        for av in arcs {
            if let Some(ao) = av.as_object() {
                fp.fp_arcs.push(FpArc {
                    start: parse_point(av.get("start").unwrap_or(&Value::Null)).unwrap_or((0.0, 0.0)),
        ...
        for rv in rects {
            if let Some(ro) = rv.as_object() {
                fp.fp_rects.push(FpRect {
                    start: parse_point(rv.get("start").unwrap_or(&Value::Null)).unwrap_or((0.0, 0.0)),
        ...
        for pv in props {
            if let Some(po) = pv.as_object() {
                ...
                fp.properties_ext.push(FpProperty {
                    ...
                    unlocked: po.get("unlocked").and_then(|x| x.as_bool()).unwrap_or(false),
                    effects: FpPropertyEffects {
                        font_size,
                        font_thickness: effects.and_then(|e| e.get("thickness"))
                            .and_then(|x| x.as_f64()).unwrap_or(0.15),
                        bold: effects.and_then(|e| e.get("bold"))
                            .and_then(|x| x.as_bool()).unwrap_or(false),
                        italic: effects.and_then(|e| e.get("italic"))
                            .and_then(|x| x.as_bool()).unwrap_or(false),
                    },
                    uuid: None,
                });
...
fn value_to_pad(v: &Value) -> Pad {
    let obj = match v.as_object() {
        Some(o) => o,
        None => return Pad {
            number: String::new(), pad_type: PadType::Smd, shape: PadShape::Rect,
            position: (0.0, 0.0, 0.0), size: (0.5, 0.5), layers: Vec::new(),
            drill: None, net: None, net_name: None, pin_function: None, pin_type: None, roundrect_rratio: None,
            ...
            remove_unused_layers: None, options: None, primitives: Vec::new(),
        },
    };
    ...
    Pad {
        ...
        options: v.get("options").and_then(|o| o.as_object()).map(|o| PadOptions {
    ...
    for pv in arr {
        ...
        match get_str(obj, "kind").unwrap_or("") {
            "gr_poly" => {
                ...
                if !pts.is_empty() {
                    out.push(PadPrimitive::GrPoly { pts, width: width(), fill: fill() });
                }
            }
            "gr_rect" => {
                if let (Some(s), Some(e)) = (pt("start"), pt("end")) {
                    out.push(PadPrimitive::GrRect { start: s, end: e, width: width(), fill: fill() });
                }
            }
            "gr_line" => {
                if let (Some(s), Some(e)) = (pt("start"), pt("end")) {
                    out.push(PadPrimitive::GrLine { start: s, end: e, width: width() });
                }
            }
            "gr_circle" => {
                if let (Some(c), Some(e)) = (pt("center"), pt("end")) {
                    out.push(PadPrimitive::GrCircle { center: c, end: e, width: width(), fill: fill() });
                }
            }
            "gr_arc" => {
                if let (Some(s), Some(m), Some(e)) = (pt("start"), pt("mid"), pt("end")) {
                    out.push(PadPrimitive::GrArc { start: s, mid: m, end: e, width: width() });
                }
            }
            "segment" => {
                if let (Some(s), Some(e)) = (pt("start"), pt("end")) {
                    out.push(PadPrimitive::Segment { start: s, end: e, width: width() });
                }
            }
            _ => {}
        }
...
fn value_to_zone(v: &Value) -> Zone {
    ...
    let net_name = obj.and_then(|o| get_str(o, "net_name")).unwrap_or("").to_string();
    let layer = obj.and_then(|o| get_str(o, "layer")).unwrap_or("F.Cu").to_string();

    let (hatch_style, hatch_pitch) = v.get("hatch")
    ...
    Zone {
        net, net_name, layer,
        hatch_style, hatch_pitch,
        pad_connect: obj.and_then(|o| get_str(o, "pad_connect")).unwrap_or("").to_string(),
        connect_pads_clearance: obj.and_then(|o| get_f64(o, "clearance")).unwrap_or(0.2),
        min_thickness: obj.and_then(|o| get_f64(o, "min_thickness")).unwrap_or(0.254),
        fill: obj.and_then(|o| o.get("fill").and_then(|v| v.as_bool())).unwrap_or(false),
        thermal_gap: obj.and_then(|o| get_f64(o, "thermal_gap")).unwrap_or(0.508),
        thermal_bridge_width: obj.and_then(|o| get_f64(o, "thermal_bridge_width")).unwrap_or(0.508),
        island_removal_mode: obj.and_then(|o| get_f64(o, "island_removal_mode")).unwrap_or(0.0) as i32,
...
fn parse_filled_polygons(v: &Value) -> Vec<FilledPolygon> {
    v.get("filled_polygons")
        .and_then(|v| v.as_array())
        .map(|arr| arr.iter().filter_map(|fv| {
...
fn value_to_graphic(v: &Value) -> Option<BoardGraphic> {
    ...
    Some(BoardGraphic { kind, layer, stroke_width, fill: obj.get("fill").and_then(|v| v.as_bool()).unwrap_or(false) })
}
