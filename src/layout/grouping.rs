...
pub fn group_by_ic(
    ...
    let ic_nets: Vec<HashSet<u32>> = ic_indices.iter().map(|&ic_idx| {
        comp_net_ids[ic_idx].iter().copied().flatten().collect()
    }).collect();
    ...
    let ic_unique_nets: Vec<HashSet<u32>> = ic_nets.iter().map(|s| {
        s.difference(&universal_nets).copied().collect()
    }).collect();
    ...
    for &cidx in &candidates {
        ...
        let best_ic = ic_indices.iter().enumerate().max_by_key(|&(gi, _)| {
            c_nets.intersection(&ic_unique_nets[gi]).count()
        }).map(|(gi, _)| gi);
...
pub fn build_peripheral_constraint(
    ...
    let is_fb_resistor = lib_u.contains(":R") && connects_special.iter().any(|(n, _)| n.contains("FB"));
