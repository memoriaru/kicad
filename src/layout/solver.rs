...
impl PlacementSolver {
    ...
    pub fn new(ic_pos: (f64, f64), ic_pin_positions: &HashMap<String, (f64, f64)>) -> Self {
        ...
        resolved.insert("ic".into(), PlacementResult { x: ic_pos.0, y: ic_pos.1, rotation: 0.0 });
        Self {
            resolved,
    ...
    pub fn register_body(&mut self, role: &str, width: f64, height: f64) {
        self.body_dims.insert(role.to_string(), BodyDims { width, height });
    }
    ...
    pub fn resolve_all(&mut self, layout: &HashMap<String, PlacementEntry>) {
        ...
        for (role, entry) in layout {
            ...
            if let PlacementEntry::Fixed { x, y } = entry {
                self.resolved.insert(role.clone(), PlacementResult { x: *x, y: *y, rotation: 0.0 });
            }
        }
        ...
        for (role, entry) in layout {
            if role == "ic" { continue; }
            match entry {
                PlacementEntry::NearPin { near_pin, max_distance_mil, side, clearance_mil } => {
                    if let Some(mut result) = self.resolve_near_pin(near_pin, *max_distance_mil, side.as_deref(), clearance_mil.unwrap_or(20.0)) {
                ...
                PlacementEntry::BetweenPins { pin_a, pin_b, offset_mil, side } => {
                    if let Some(result) = self.resolve_between_pins(pin_a, pin_b, offset_mil.unwrap_or(0.0), side.as_deref()) {
                        self.resolved.insert(role.clone(), result);
                    }
                }
                _ => {}
        ...
        for (role, entry) in layout {
            if role == "ic" { continue; }
            if let PlacementEntry::OffsetFrom { offset_from, dx_mil, dy_mil } = entry {
    ...
    pub fn get_or(&self, role: &str, fallback: (f64, f64, f64)) -> PlacementResult {
        self.resolved.get(role).copied().unwrap_or(PlacementResult { x: fallback.0, y: fallback.1, rotation: fallback.2 })
    }
    ...
    fn resolve_between_pins(
        ...
        let (sign, rotation) = match side {
            ...
            _ => {
                ...
                if dot > 0.0 { (-1.0, 0.0) } else { (1.0, 0.0) }
            }
        };
        ...
        Some(PlacementResult { x, y, rotation: angle + rotation })
    }

    fn resolve_offset_from(&self, target: &str, dx_mil: f64, dy_mil: f64) -> Option<PlacementResult> {
        ...
        for _ in 0..max_iterations {
            ...
            for i in 0..roles.len() {
                for j in (i + 1)..roles.len() {
                    ...
                    let (Some(pi), Some(pj)) = (pos_i, pos_j) else { continue };

                    let dim_i = self.body_dims.get(ri).copied().unwrap_or(BodyDims { width: 1.0, height: 1.0 });
                    let dim_j = self.body_dims.get(rj).copied().unwrap_or(BodyDims { width: 1.0, height: 1.0 });

                    if rects_overlap((pi.x, pi.y, dim_i.width, dim_i.height), (pj.x, pj.y, dim_j.width, dim_j.height)) {
                        any_collision = true;
                        let dx = pj.x - pi.x;
                        let dy = pj.y - pi.y;
...
mod tests {
    ...
    fn test_near_pin_left() {
        ...
        layout.insert("c_in".into(), PlacementEntry::NearPin {
        ...
        assert!(pos.x < 38.0, "c_in should be left of VIN pin, got x={}", pos.x);
        assert!((pos.y - 0.0).abs() < 1.0, "c_in should be at similar y as VIN pin");
    }
    ...
    fn test_near_pin_right() {
        ...
        let mut layout = HashMap::new();
        layout.insert("c_out".into(), PlacementEntry::NearPin {
        ...
        assert!(pos.x > 42.0, "c_out should be right of VOUT pin, got x={}", pos.x);
    }
    ...
    fn test_between_pins() {
        ...
        let mut layout = HashMap::new();
        layout.insert("r_fb".into(), PlacementEntry::BetweenPins {
            pin_a: "ic.VOUT".into(),
            pin_b: "ic.FB".into(),
        ...
        assert!((pos.x - mid_x).abs() < 5.0, "r_fb should be near midpoint x, got {}", pos.x);
    }
    ...
    fn test_offset_from() {
        ...
        layout.insert("r2".into(), PlacementEntry::OffsetFrom {
            offset_from: "r1".into(),
            dx_mil: 0.0,
        ...
        assert!((r2.y - expected_y).abs() < 0.01, "r2 y offset should be ~{}, got {}", expected_y, r2.y);
    ...
    fn test_collision_avoidance() {
        ...
        assert!(dist > 2.0, "Colliding components should be separated, dist={}", dist);
    }
}
