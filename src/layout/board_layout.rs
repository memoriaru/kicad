...
pub fn auto_layout(schematic: &Schematic) -> BoardLayout {
    ...
    if n == 0 {
        return BoardLayout { sections: Vec::new(), positions: HashMap::new(), dividers: Vec::new(), section_labels: Vec::new() };
    }

    // Step 1: Classify all components
    let comp_kinds: Vec<ComponentKind> = schematic.components.iter()
        .map(|c| classify_component(&c.lib_id))
        .collect();

    // Step 2: Build net lookup tables
    let net_id_to_name: HashMap<u32, String> = schematic.nets.iter()
        .map(|n| (n.id, n.name.clone()))
        .collect();
    let net_name_to_id: HashMap<String, u32> = schematic.nets.iter()
        .map(|n| (n.name.clone(), n.id))
        .collect();
    let net_kinds: HashMap<u32, NetKind> = schematic.nets.iter()
        .map(|n| (n.id, classify_net(&n.name)))
        .collect();

    // Step 3: Build component → connected net IDs mapping
    let comp_nets: Vec<HashSet<u32>> = schematic.components.iter()
        .map(|comp| {
            comp.pins.iter()
                .filter_map(|p| p.net_id)
                .collect()
        })
        .collect();
    ...
    let sorted_anchors = sort_anchors_by_power(&anchors, &comp_nets, &power_order, &net_kinds, schematic);

    // Step 7: Group peripherals around anchors
    let mut sections = group_into_sections(
        &sorted_anchors, &comp_nets, &comp_kinds, n,
    );
    ...
    for section in &mut sections {
        place_section_components(
            section, schematic, &comp_kinds, &comp_nets,
            &net_id_to_name, &net_name_to_id, &net_kinds,
            &mut positions,
        );
    ...
    BoardLayout { sections, positions, dividers, section_labels }
}
    ...
    for (i, comp) in schematic.components.iter().enumerate() {
        ...
        let kind = match comp_kinds[i] {
            ...
            ComponentKind::Transistor => {
                if comp.pins.len() >= 3 { Some(AnchorKind::Transistor) } else { None }
            }
...
fn classify_by_heuristic(lib_id: &str, pin_count: usize) -> Option<AnchorKind> {
    ...
    let ic_patterns = [
        "SY81", "RT9", "FP62", "CH34", "SN65", "ADS", "STM32", "ESP32",
        "LM35", "LM393", "AMS1117", "TPS", "MP15", "XL15",
        "74HC", "74LS", "CD40",
    ];
...
fn analyze_power_flow(
    ...
    let power_nets: Vec<(u32, String)> = schematic.nets.iter()
        .filter_map(|n| {
    ...
    let net_id_to_name: HashMap<u32, &str> = schematic.nets.iter()
        .filter_map(|n| Some((n.id, n.name.as_str())))
        .collect();

    for comp in &schematic.components {
        let comp_power_nets: Vec<(u32, &str)> = comp.pins.iter()
            .filter_map(|pin| {
        ...
        if comp_power_nets.len() >= 2 {
            ...
            sorted.sort_by(|a, b| power_net_rank_heuristic(a.1).cmp(&power_net_rank_heuristic(b.1)));
...
fn power_net_rank_heuristic(name: &str) -> usize {
    ...
    if lu.contains("BAT_IN") || lu == "VIN" || lu.contains("VBUS") { return 0; }
    if lu.contains("FUSED") || lu.contains("PROT") { return 1; }
    if lu.contains("7V4") || lu.contains("BAT") { return 2; }
    // Higher voltages → mid rank
    if lu.contains("12V") { return 3; }
    if lu.contains("5V") { return 4; }
    if lu.contains("3V3") || lu.contains("3.3") { return 5; }
    if lu.contains("1V8") || lu.contains("1.8") { return 6; }
...
fn topological_sort_power_nets(
    ...
    while !queue.is_empty() {
        queue.sort();
        let next_queue: Vec<u32> = queue.drain(..).collect();
...
fn sort_anchors_by_power(
    ...
    sorted.sort_by_key(|a| {
        ...
        let power_ranks: Vec<usize> = nets.iter()
            .filter_map(|nid| {
    ...
    let connector_indices: Vec<usize> = sorted.iter().enumerate()
...
fn group_into_sections(
    ...
    let mut sections: Vec<Section> = anchors.iter().map(|a| {
        Section {
    ...
    let _gnd_net_ids: HashSet<u32> = comp_nets.iter().flatten()
        .filter(|&&_nid| {
    ...
    let anchor_nets: Vec<HashSet<u32>> = anchors.iter()
        .map(|a| comp_nets[a.index].clone())
        .collect();

    // Find universal nets (shared by all anchors) — exclude from scoring
    let universal: HashSet<u32> = if anchor_nets.len() > 1 {
        anchor_nets.iter().skip(1).fold(anchor_nets[0].clone(), |acc, s| {
    ...
    for i in 0..total {
        ...
        if matches!(comp_kinds[i], ComponentKind::Connector) { continue; }
...
fn arrange_sections_by_bboxes(sections: &mut [Section]) -> Vec<DividerLine> {
    ...
    let connector_count = sections.iter().filter(|s| s.anchor_kind == AnchorKind::Connector).count();
    ...
    for (si, section) in sections.iter_mut().enumerate() {
        ...
        let is_output_connector = section.anchor_kind == AnchorKind::Connector
            && conn_placed == connector_count - 1;
    ...
    for (ri, row) in rows.iter().enumerate() {
        ...
        let row_top = row.iter().map(|(_, _, y, _, _)| *y).fold(f64::MAX, f64::min);
        let row_bottom = row.iter().map(|(_, _, y, _, h)| y + h).fold(f64::MIN, f64::max);
        let row_left = row.iter().map(|(_, x, _, _, _)| *x).fold(f64::MAX, f64::min);
        let row_right = row.iter().map(|(_, x, _, w, _)| x + w).fold(f64::MIN, f64::max);
        ...
        if ri < rows.len() - 1 {
            let next_row_top = rows[ri + 1].iter().map(|(_, _, y, _, _)| *y).fold(f64::MAX, f64::min);
...
fn place_section_components(
    ...
    for pin in &anchor.pins {
        ...
        if !matches!(net_kinds.get(&net_id), Some(NetKind::Power)) { continue };
    ...
    if section.anchor_kind != AnchorKind::Ic {
        place_non_ic_section(section, schematic, comp_kinds, comp_nets,
            net_id_to_name, net_kinds, positions);
        return;
    }
    ...
    for &mi in &section.member_indices {
        ...
        let connects_gate = m_nets.iter().any(|&nid| {
            net_id_to_name.get(&nid).map(|n| n.to_uppercase().contains("GATE")).unwrap_or(false)
        });
        let connects_fb = m_nets.iter().any(|&nid| {
            net_id_to_name.get(&nid).map(|n| n.to_uppercase().contains("FB")).unwrap_or(false)
        });
        let connects_bst = m_nets.iter().any(|&nid| {
            net_id_to_name.get(&nid).map(|n| n.to_uppercase().contains("BST")).unwrap_or(false)
        });

        match kind {
            ComponentKind::Capacitor => {
                if connects_bst { bst_caps.push(mi); }
                else if connects_in { input_caps.push(mi); }
                else if connects_out { output_caps.push(mi); }
                else { others.push(mi); }
            }
            ...
            ComponentKind::Resistor => {
                if connects_fb { fb_resistors.push(mi); }
                else if connects_gate { gate_resistors.push(mi); }
                else if connects_in || connects_out { pull_resistors.push(mi); }
                else { others.push(mi); }
            }
            _ => { others.push(mi); }
        }
...
fn place_remaining(
    ...
    let max_x = sections.iter().map(|s| s.x + s.width).fold(0.0f64, f64::max);
...
fn estimate_body_half_height(lib_id: &str, pin_count: usize) -> f64 {
    ...
    if is_dual {
        ...
        let left = (pin_count + 1) / 2;
        let right = pin_count - left;
        let max_per_side = left.max(right).max(1);
        let half_span = if max_per_side > 1 { ((max_per_side - 1) as f64 / 2.0) * spacing } else { 0.0 };
        half_span + 2.54 + label_room
    } else {
        // 2-pin: pin tip distance + global label room
        let lu = lib_id.to_uppercase();
        let pin_tip = if lu.contains("CP") || lu.contains("ELECTRO") { 5.08 }
                      else { 5.81 };
        pin_tip + label_room
    }
}
    ...
    match section.anchor_kind {
        AnchorKind::Connector => {
            ...
            if anchor.reference.starts_with('J') {
                ...
                if num <= 1 { "Input".to_string() } else { "Output".to_string() }
            } else {
                "Connector".to_string()
            }
        }
        ...
        AnchorKind::Protection => {
            if lib.contains("PPTC") { "Over-current Protection".to_string() }
            else { "Protection".to_string() }
        }
