...
impl SexprGenerator {
    ...
    fn generate_symbol_def(&mut self, output: &mut String, symbol: &Symbol) {
        ...
        if symbol.units.is_empty() {
            if !symbol.graphics.is_empty() {
                ...
                let groups: Vec<(String, Vec<&GraphicElement>)> = if symbol.is_power || (has_pins && !has_graphics) {
                    vec![(format!("{}_0_1", short), symbol.graphics.iter().collect())]
                } else if has_pins && has_graphics {
                    vec![
                        (format!("{}_0_1", short), symbol.graphics.iter().filter(|g| !is_pin(g)).collect()),
                        (format!("{}_1_1", short), symbol.graphics.iter().filter(|g| is_pin(g)).collect()),
                    ]
                } else {
                    vec![(format!("{}_0_1", short), symbol.graphics.iter().collect())]
                };
        ...
        if pin_count > 32 && pin_count % 4 == 0 {
            ...
            for (i, pin) in symbol.pins.iter().enumerate() {
                let (x, y, rot) = if i < per_side {
                    (-(body_half + pin_length), half_span - i as f64 * spacing, 0.0)
                } else if i < 2 * per_side {
                    let j = i - per_side;
                    (-half_span + j as f64 * spacing, -(body_half + pin_length), 90.0)
                } else if i < 3 * per_side {
                    let j = i - 2 * per_side;
                    (body_half + pin_length, -half_span + j as f64 * spacing, 180.0)
                } else {
                    let j = i - 3 * per_side;
                    (half_span - j as f64 * spacing, body_half + pin_length, 270.0)
                };
    ...
    fn gen_inductor_unit(
        ...
        self.indent_level += 1;
        let pin1 = Self::get_pin_or_default(symbol, 0, "1");
        self.gen_kicad_pin(output, "passive", &pin1.number, &pin1.number, 0.0, 5.81, 270.0, 3.27);
        let pin2 = Self::get_pin_or_default(symbol, 1, "2");
        self.gen_kicad_pin(output, "passive", &pin2.number, &pin2.number, 0.0, -5.81, 90.0, 3.27);
        self.indent_level -= 1;
        self.write_line(output, ")");
    }
