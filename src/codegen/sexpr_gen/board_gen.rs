...
impl BoardSexprGenerator {
    ...
    pub fn with_config(config: BoardSexprConfig) -> Self {
        Self { config, indent_level: 0, effective_version: KicadVersion::V7, uuid_counter: 0 }
    }

    pub fn generate(&mut self, board: &Board) -> Result<String> {
        // Auto-detect version from board IR
        self.effective_version = self.config.kicad_version.unwrap_or_else(|| {
        ...
        self.line(&mut out, &format!("(generator \"{}\")", Self::esc(&board.generator)));
        ...
        let (mut pw, mut ph) = if board.paper.starts_with("User ") {
            ...
            (it.next().and_then(|v| v.parse::<f64>().ok()).unwrap_or(297.0),
             it.next().and_then(|v| v.parse::<f64>().ok()).unwrap_or(210.0))
        ...
        let net_names: std::collections::HashMap<u32, String> = board.nets.iter()
    ...
    fn fmt_num(n: f64) -> String {
        ...
        if (n - n.round()).abs() < 1e-9 {
            ...
            format!("{}", n).trim_end_matches('0').trim_end_matches('.').to_string()
        }
    }
    ...
    fn fmt_at(x: f64, y: f64, r: f64) -> String {
        if r == 0.0 {
            ...
            format!("(at {} {} {})", Self::fmt_num(x), Self::fmt_num(y), Self::fmt_num(r))
        }
    }
    ...
    fn uuid(&mut self) -> String {
        ...
        let mut z = 0x9E3779B97F4A7C15u64
            ^ self.uuid_counter.wrapping_mul(0xBF58476D1CE4E5B9);
        ...
        for i in 0..8 { bytes[i] = ((z >> (i * 8)) & 0xFF) as u8; }
        for i in 0..8 { bytes[8 + i] = ((h2 >> (i * 8)) & 0xFF) as u8; }
    ...
    fn write_general(&mut self, out: &mut String, board: &Board) {
        ...
        self.indent_level += 1;
        self.line(out, &format!("(thickness {})", Self::fmt_num(board.general.thickness)));
        self.indent_level -= 1;
        self.line(out, ")");
    }
    ...
    fn write_title_block(&mut self, out: &mut String, tb: &BoardTitleBlock) {
        ...
        self.indent_level += 1;
        if let Some(ref t) = tb.title { self.line(out, &format!("(title \"{}\")", Self::esc(t))); }
        if let Some(ref d) = tb.date { self.line(out, &format!("(date \"{}\")", d)); }
        if let Some(ref r) = tb.rev { self.line(out, &format!("(rev \"{}\")", r)); }
        if let Some(ref c) = tb.company { self.line(out, &format!("(company \"{}\")", Self::esc(c))); }
    ...
    fn write_layers(&mut self, out: &mut String, layers: &[LayerDef]) {
        ...
        self.indent_level += 1;
        for l in layers {
            self.line(out, &format!("({} \"{}\" {})", l.ordinal, l.name, l.layer_type));
        }
        self.indent_level -= 1;
        self.line(out, ")");
    }
    ...
    fn write_setup(&mut self, out: &mut String, setup: &BoardSetup) {
        ...
        if !setup.stackup.is_empty() {
            ...
            for sl in &setup.stackup {
                self.line(out, &format!("(layer \"{}\" (type {}))", sl.name, sl.layer_type));
            }
            self.indent_level -= 1;
            self.line(out, ")");
        }

        self.line(out, &format!("(pad_to_mask_clearance {})", Self::fmt_num(setup.pad_to_mask_clearance)));
        self.line(out, &format!("(pad_to_paste_clearance {})", Self::fmt_num(setup.pad_to_paste_clearance)));
        self.line(out, &format!("(pad_to_paste_clearance_ratio {})", Self::fmt_num(setup.pad_to_paste_clearance_ratio)));

        if let Some((x, y)) = setup.aux_axis_origin {
            self.line(out, &format!("(aux_axis_origin {} {})", Self::fmt_num(x), Self::fmt_num(y)));
        }
        if let Some((x, y)) = setup.grid_origin {
            self.line(out, &format!("(grid_origin {} {})", Self::fmt_num(x), Self::fmt_num(y)));
        }
    ...
    fn write_footprint(&mut self, out: &mut String, fp: &Footprint, net_names: &std::collections::HashMap<u32, String>) {
        ...
        if !fp.properties_ext.is_empty() {
            ...
            let has_ref_t = fp.fp_texts.iter().any(|t| t.text_type == FpTextType::Reference);
        ...
        if let Some(margin) = fp.solder_mask_margin {
            self.line(out, &format!("(solder_mask_margin {})", Self::fmt_num(margin)));
        }
        if let Some(margin) = fp.solder_paste_margin {
            self.line(out, &format!("(solder_paste_margin {})", Self::fmt_num(margin)));
        }
        if let Some(ratio) = fp.solder_paste_ratio {
            self.line(out, &format!("(solder_paste_ratio {})", Self::fmt_num(ratio)));
        }
        ...
        if let Some(tw) = fp.thermal_bridge_width {
            self.line(out, &format!("(thermal_bridge_width {})", Self::fmt_num(tw)));
        }
        if let Some(ta) = fp.thermal_bridge_angle {
            self.line(out, &format!("(thermal_bridge_angle {})", Self::fmt_num(ta)));
        }
        ...
        for ft in &fp.fp_texts {
            self.write_fp_text_sized(out, Self::fp_text_type_str(ft.text_type), &ft.text, ft.position, &ft.layer, ft.font_size, 0.15);
        }
    ...
    fn write_model(&mut self, out: &mut String, model: &Model3D) {
        ...
        self.line(out, &format!("(offset (xyz {} {} {}))", Self::fmt_num(ox), Self::fmt_num(oy), Self::fmt_num(oz)));
        let (sx, sy, sz) = model.scale;
        self.line(out, &format!("(scale (xyz {} {} {}))", Self::fmt_num(sx), Self::fmt_num(sy), Self::fmt_num(sz)));
        let (rx, ry, rz) = model.rotate;
        self.line(out, &format!("(rotate (xyz {} {} {}))", Self::fmt_num(rx), Self::fmt_num(ry), Self::fmt_num(rz)));
        self.indent_level -= 1;
        self.line(out, ")");
    }
    ...
    fn write_fp_text(&mut self, out: &mut String, text_type: &str, text: &str, pos: (f64, f64, f64), layer: &str) {
        self.write_fp_text_sized(out, text_type, text, pos, layer, (1.0, 1.0), 0.15);
    }

    fn write_fp_text_sized(&mut self, out: &mut String, text_type: &str, text: &str, pos: (f64, f64, f64), layer: &str, font_size: (f64, f64), thickness: f64) {
        self.line(out, &format!("(fp_text {} \"{}\"", text_type, Self::esc(text)));
        self.indent_level += 1;
        let (x, y, r) = pos;
        self.line(out, &Self::fmt_at(x, y, r));
        self.line(out, &format!("(layer \"{}\")", layer));
        self.maybe_uuid(out);
        self.line(out, &format!("(effects (font (size {} {}) (thickness {})))", Self::fmt_num(font_size.0), Self::fmt_num(font_size.1), Self::fmt_num(thickness)));
        self.indent_level -= 1;
        self.line(out, ")");
    }
    ...
    fn write_fp_line(&mut self, out: &mut String, fl: &FpLine) {
        ...
        let (sx, sy) = fl.start;
        let (ex, ey) = fl.end;
        self.line(out, &format!("(start {} {})", Self::fmt_num(sx), Self::fmt_num(sy)));
        self.line(out, &format!("(end {} {})", Self::fmt_num(ex), Self::fmt_num(ey)));
        self.line(out, &format!("(stroke (width {}) (type default))", Self::fmt_num(fl.stroke_width)));
    ...
    fn write_fp_circle(&mut self, out: &mut String, fc: &FpCircle) {
        ...
        let (cx, cy) = fc.center;
        let (ex, ey) = fc.end;
        self.line(out, &format!("(center {} {})", Self::fmt_num(cx), Self::fmt_num(cy)));
        self.line(out, &format!("(end {} {})", Self::fmt_num(ex), Self::fmt_num(ey)));
        self.line(out, &format!("(stroke (width {}) (type default))", Self::fmt_num(fc.stroke_width)));
    ...
    fn write_fp_arc(&mut self, out: &mut String, fa: &FpArc) {
        ...
        let (ex, ey) = fa.end;
        self.line(out, &format!("(start {} {})", Self::fmt_num(sx), Self::fmt_num(sy)));
        self.line(out, &format!("(mid {} {})", Self::fmt_num(mx), Self::fmt_num(my)));
        self.line(out, &format!("(end {} {})", Self::fmt_num(ex), Self::fmt_num(ey)));
        self.line(out, &format!("(stroke (width {}) (type default))", Self::fmt_num(fa.stroke_width)));
    ...
    fn write_fp_rect(&mut self, out: &mut String, fr: &FpRect) {
        ...
        self.line(out, &format!("(start {} {})", Self::fmt_num(sx), Self::fmt_num(sy)));
        self.line(out, &format!("(end {} {})", Self::fmt_num(ex), Self::fmt_num(ey)));
        self.line(out, &format!("(stroke (width {}) (type default))", Self::fmt_num(fr.stroke_width)));
    ...
    fn write_fp_poly(&mut self, out: &mut String, fp: &FpPoly) {
        ...
        self.indent_level -= 1;
        self.line(out, ")");
        self.line(out, &format!("(stroke (width {}) (type default))", Self::fmt_num(fp.stroke_width)));
        self.line(out, &format!("(layer \"{}\")", fp.layer));
        if fp.fill {
            self.line(out, "(fill solid)");
        }
    ...
    fn write_pad(&mut self, out: &mut String, pad: &Pad, net_names: &std::collections::HashMap<u32, String>) {
        self.line(out, &format!("(pad \"{}\" {} {}", pad.number, pad.pad_type.as_str(), pad.shape.as_str()));
        self.indent_level += 1;
        let (x, y, r) = pad.position;
        self.line(out, &Self::fmt_at(x, y, r));
        let (w, h) = pad.size;
        self.line(out, &format!("(size {} {})", Self::fmt_num(w), Self::fmt_num(h)));

        if let Some(ref drill) = pad.drill {
            self.line(out, &format!("(drill {})", Self::fmt_num(drill.diameter)));
            if let Some((ox, oy)) = drill.offset {
                self.line(out, &format!("(offset {} {})", Self::fmt_num(ox), Self::fmt_num(oy)));
            }
        }

        let layers_str = pad.layers.iter()
            .map(|l| format!("\"{}\"", l))
            .collect::<Vec<_>>()
            .join(" ");
        ...
        if let Some(remove_unused_layers) = pad.remove_unused_layers {
            self.line(out, &format!("(remove_unused_layers {})", if remove_unused_layers { "yes" } else { "no" }));
        }
        if let Some(roundrect_rratio) = pad.roundrect_rratio {
            self.line(out, &format!("(roundrect_rratio {})", Self::fmt_num(roundrect_rratio)));
        }
    ...
    fn write_pad_primitive(&mut self, out: &mut String, prim: &crate::ir::board::PadPrimitive) {
        ...
        let fmt_pts = |pts: &[(f64, f64)]| {
            pts.iter().map(|(x, y)| format!("(xy {} {})", Self::fmt_num(*x), Self::fmt_num(*y))).collect::<Vec<_>>().join(" ")
        };
        match prim {
            ...
            P::GrRect { start, end, width, fill } => {
                ...
                self.indent_level += 1;
                self.line(out, &format!("(start {} {})", Self::fmt_num(start.0), Self::fmt_num(start.1)));
                self.line(out, &format!("(end {} {})", Self::fmt_num(end.0), Self::fmt_num(end.1)));
                self.line(out, &format!("(width {})", Self::fmt_num(*width)));
                self.line(out, &format!("(fill {})", if *fill { "yes" } else { "no" }));
                self.indent_level -= 1;
                self.line(out, ")");
            ...
            P::GrCircle { center, end, width, fill } => {
                ...
                self.indent_level += 1;
                self.line(out, &format!("(center {} {})", Self::fmt_num(center.0), Self::fmt_num(center.1)));
                self.line(out, &format!("(end {} {})", Self::fmt_num(end.0), Self::fmt_num(end.1)));
                self.line(out, &format!("(width {})", Self::fmt_num(*width)));
                self.line(out, &format!("(fill {})", if *fill { "yes" } else { "no" }));
                self.indent_level -= 1;
            ...
            P::GrArc { start, mid, end, width } => {
                ...
                self.indent_level += 1;
                self.line(out, &format!("(start {} {})", Self::fmt_num(start.0), Self::fmt_num(start.1)));
                self.line(out, &format!("(mid {} {})", Self::fmt_num(mid.0), Self::fmt_num(mid.1)));
                self.line(out, &format!("(end {} {})", Self::fmt_num(end.0), Self::fmt_num(end.1)));
                self.line(out, &format!("(width {})", Self::fmt_num(*width)));
                self.indent_level -= 1;
                self.line(out, ")");
    ...
    fn write_segment(&mut self, out: &mut String, seg: &Segment, net_names: &std::collections::HashMap<u32, String>) {
        self.line(out, "(segment");
        self.indent_level += 1;
        let (sx, sy) = seg.start;
        let (ex, ey) = seg.end;
        ...
        match net_names.get(&seg.net) {
            Some(name) if !name.is_empty() => self.line(out, &format!("(net \"{}\")", Self::esc(name))),
    ...
    fn write_via(&mut self, out: &mut String, via: &Via, net_names: &std::collections::HashMap<u32, String>) {
        self.line(out, "(via");
        self.indent_level += 1;
        let (x, y) = via.at;
        ...
        let layers_str = via.layers.iter()
            .map(|l| format!("\"{}\"", l))
            .collect::<Vec<_>>()
            .join(" ");
        self.line(out, &format!("(layers {})", layers_str));
    ...
    fn write_zone(&mut self, out: &mut String, zone: &Zone) {
        ...
        self.maybe_uuid(out);
        self.line(out, &format!("(hatch {} {})", zone.hatch_style, Self::fmt_num(zone.hatch_pitch)));
        ...
        if zone.pad_connect.is_empty() {
            self.line(out, &format!("(connect_pads (clearance {}))", Self::fmt_num(zone.connect_pads_clearance)));
        } else {
            self.line(out, &format!("(connect_pads {} (clearance {}))", zone.pad_connect, Self::fmt_num(zone.connect_pads_clearance)));
        }

        self.line(out, &format!("(min_thickness {})", Self::fmt_num(zone.min_thickness)));

        // keepout
        if let Some(ref ko) = zone.keepout {
            self.line(out, "(keepout");
            self.indent_level += 1;
            self.line(out, &format!("(tracks {})", if ko.tracks { "allowed" } else { "not_allowed" }));
            self.line(out, &format!("(vias {})", if ko.vias { "allowed" } else { "not_allowed" }));
            self.line(out, &format!("(pads {})", if ko.pads { "allowed" } else { "not_allowed" }));
            self.line(out, &format!("(copperpour {})", if ko.copperpour { "allowed" } else { "not_allowed" }));
            self.line(out, &format!("(footprints {})", if ko.footprints { "allowed" } else { "not_allowed" }));
            self.indent_level -= 1;
            self.line(out, ")");
        }
        ...
        for fpoly in &zone.filled_polygons {
            if !fpoly.points.is_empty() {
                self.line(out, &format!("(filled_polygon"));
                self.indent_level += 1;
    ...
    fn write_board_graphic(&mut self, out: &mut String, gr: &BoardGraphic) {
        match &gr.kind {
            BoardGraphicKind::Line { start, end } => {
                ...
                self.line(out, &format!("(start {} {})", Self::fmt_num(sx), Self::fmt_num(sy)));
                self.line(out, &format!("(end {} {})", Self::fmt_num(ex), Self::fmt_num(ey)));
                self.line(out, &format!("(stroke (width {}) (type default))", Self::fmt_num(gr.stroke_width)));
            ...
            BoardGraphicKind::Text { text, position, font_size } => {
                ...
                self.line(out, &format!("(layer \"{}\")", gr.layer));
                self.maybe_uuid(out);
                self.line(out, &format!("(effects (font (size {} {})))", Self::fmt_num(*font_size), Self::fmt_num(*font_size)));
                self.indent_level -= 1;
                self.line(out, ")");
            }
...
mod tests {
    ...
    fn test_board_with_zone() {
        ...
        assert!(output.contains("(fill yes (thermal_gap 0.508) (thermal_bridge_width 0.508) (island_removal_mode 0))"));
    }
	...
	(footprint "Test:F" (layer "F.Cu") (at 0 0)
        ...
        assert!(out.contains("(clearance outline)"), "options.clearance lost:\n{}", out);
        assert!(out.contains("(anchor rect)"), "options.anchor lost:\n{}", out);
        assert!(out.contains("(primitives"), "primitives lost:\n{}", out);
        assert!(out.contains("(gr_poly"), "gr_poly lost:\n{}", out);
        assert!(out.contains("(xy 3.8625 0.8665)"), "poly points lost:\n{}", out);
        assert!(out.contains("(remove_unused_layers no)"), "remove_unused_layers lost:\n{}", out);

        // 二次往返：新字段必须稳定（不再丢失、不变形）
        let board2 = crate::parse_board(&out).unwrap();
        let out2 = BoardSexprGenerator::new().generate(&board2).unwrap();
        for key in ["(options", "(clearance outline)", "(anchor rect)", "(gr_poly", "(remove_unused_layers no)"] {
