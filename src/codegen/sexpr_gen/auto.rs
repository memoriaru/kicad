...
impl SexprGenerator {
    ...
    pub fn generate_auto_labels(
        ...
        for net_id in &sorted_net_ids {
            ...
            match render {
                RenderHint::Wire => self.render_net_wires(output, *net_id, net_name, pins, &mut all_label_positions, &mut wire_segments, &all_pins_with_net, &component_bodies),
                RenderHint::Label => self.render_net_labels(output, *net_id, net_name, pins, &mut all_label_positions, &mut wire_segments, &all_pins_with_net, &component_bodies),
        ...
        if pins.len() >= 2 {
            ...
            for _ in 1..sorted_indices.len() {
                ...
                for (cand_idx, _) in &candidates {
                    ...
                    if self.emit_l_wire(output, p1, p2, net_id, wire_segments, all_pins_with_net, component_bodies) {
    ...
    fn emit_l_wire(
        ...
        if (y1 - y2).abs() < COORD_TOL {
            ...
            if !conflict_a {
                self.emit_wire_segment(output, x1, y1, corner_a.0, corner_a.1, net_id, wire_segments);
                self.emit_wire_segment(output, corner_a.0, corner_a.1, x2, y2, net_id, wire_segments);
                return true;
            } else if !conflict_b {
                self.emit_wire_segment(output, x1, y1, corner_b.0, corner_b.1, net_id, wire_segments);
                self.emit_wire_segment(output, corner_b.0, corner_b.1, x2, y2, net_id, wire_segments);
                return true;
            }

            // Both L-shapes conflict — try Z-shape (3-segment) routing
            if self.emit_z_wire(output, p1, p2, net_id, wire_segments, all_pins_with_net, component_bodies) {
                return true;
            }
            return false;
        }
    ...
    fn collect_component_bodies(schematic: &crate::ir::Schematic) -> Vec<BodyRect> {
        ...
        for comp in &schematic.components {
                ...
                || lib_id.starts_with("device:l")
            {
                ...
                || lib_id.starts_with("device:led")
            {
                (1.27, 0.635)
            } else if lib_id.contains("connector")
    ...
    fn manhattan_segment_intersects_rect(sx: f64, sy: f64, ex: f64, ey: f64, rect: &BodyRect) -> bool {
    ...
    fn emit_z_wire(
        ...
        for x_mid in &x_mids {
            ...
            if Self::check_z_conflicts(seg1, seg2, seg3, net_id, all_pins_with_net, wire_segments, component_bodies) {
                continue;
            }

            self.emit_wire_segment(output, seg1.0.0, seg1.0.1, seg1.1.0, seg1.1.1, net_id, wire_segments);
            self.emit_wire_segment(output, seg2.0.0, seg2.0.1, seg2.1.0, seg2.1.1, net_id, wire_segments);
            self.emit_wire_segment(output, seg3.0.0, seg3.0.1, seg3.1.0, seg3.1.1, net_id, wire_segments);
            return true;
        }
    ...
    pub(super) fn generate_power_flags(
        ...
        for (net_id, types) in &net_pin_types {
            ...
            let has_power_out = types.iter().any(|t| *t == "power_out");
            let has_signal_output = types.iter().any(|t| {
                matches!(*t, "output" | "bidirectional")
            });
