...
use clap::Parser;
use kicad_json5::{InputFormat, BoardSexprGenerator, Json5Generator, KicadVersion, Lexer, Parser as SExprParser, SexprGenerator};
...
fn looks_like_board(source: &str) -> bool {
    ["footprints", "segments"].iter().any(|key| {
        source.contains(&format!("{key}:")) || source.contains(&format!("\"{key}\":"))
    })
}
    ...
    if args.validate {
        match input_format {
            InputFormat::PcbSexpr => {
                ...
                if args.verbose {
                    eprintln!("Parsed {} footprints, {} nets, {} segments",
                        board.footprints.len(), board.nets.len(), board.segments.len());
                }
            }
            InputFormat::Json5 => {
...
fn run_pcb(args: &Args, source: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if matches!(output_ext, Some("kicad_pcb") | None | Some("sexpr")) {
        let vd = match args.dialect.as_str() {
            ...
            other => return Err(format!("unknown --dialect '{other}' (auto|huaqiu|official)").into()),
        };
        if args.verbose {
    ...
    let output_content = match output_ext {
        Some("json5" | "json") => {
            kicad_json5::generate_board_json5(&board)?
        }
        Some("kicad_pcb") | None => {
