...
pub fn generate_netlist(schematic: &Schematic) -> Result<String> {
    ...
    for comp in &schematic.components {
        ...
        if let Some(lib) = parse_lib_name(&comp.lib_id) {
            out.push_str(&format!(
                "      (libsource (lib \"{}\") (part \"{}\"))\n",
                lib,
                parse_part_name(&comp.lib_id).unwrap_or(&comp.value)
            ));
        }
    ...
    let net_names: BTreeMap<u32, String> = schematic
        .nets
        .iter()
    ...
    let mut sorted_nets: Vec<(u32, &String)> =
        net_names.iter().map(|(id, name)| (*id, name)).collect();
