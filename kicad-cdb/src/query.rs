...
impl ComponentDb {
    ...
    pub fn query_by_parameter_range(
        ...
        let comps = match (min, max) {
            (Some(lo), Some(hi)) => {
                stmt.query_map(params![param_name, lo, hi], self.map_component())?
            }
    ...
    pub fn query_by_parameter_exact(
        ...
        let comps = stmt
            .query_map(
                params![category_name, param_name, value],
                self.map_component(),
            )?
            .collect::<Result<Vec<_>, _>>()?;
        Ok(comps)
    ...
    pub fn query_by_text_parameter(
        &self,
        param_name: &str,
        text_contains: &str,
    ) -> Result<Vec<Component>> {
        ...
        let comps = stmt
            .query_map(params![param_name, pattern], self.map_component())?
            .collect::<Result<Vec<_>, _>>()?;
        Ok(comps)
    }
    ...
    pub fn query_in_stock(&self) -> Result<Vec<Component>> {
        ...
        let comps = stmt
            .query_map([], self.map_component())?
            .collect::<Result<Vec<_>, _>>()?;
        Ok(comps)
    }

    /// Query by multiple parameter ranges (intersection)
    pub fn query_by_multiple_params(
        &self,
        filters: &[(&str, Option<f64>, Option<f64>)],
    ) -> Result<Vec<Component>> {
        if filters.is_empty() {
            return Ok(vec![]);
        }
        ...
        let comps = stmt
            .query_map(param_refs.as_slice(), self.map_component())?
            .collect::<Result<Vec<_>, _>>()?;
        Ok(comps)
    }
    ...
    pub fn get_component_with_params(
        &self,
        id: i64,
    ) -> Result<Option<(Component, Vec<Parameter>)>> {
        match self.get_component(id)? {
            Some(comp) => {
    ...
    pub fn search(&self, query: &str) -> Result<Vec<Component>> {
        ...
        let comps = stmt
            .query_map(params![pattern], self.map_component())?
            .collect::<Result<Vec<_>, _>>()?;
        Ok(comps)
    }
    ...
    pub fn list_parameter_names(&self, category: Option<&str>) -> Result<Vec<String>> {
        let sql = match category {
            ...
            None => "SELECT DISTINCT name FROM parameters ORDER BY name",
        };
        let mut stmt = self.conn.prepare(sql)?;
