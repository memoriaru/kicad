...
fn test_import_from_json() {
    ...
    assert_eq!(
        comp.kicad_symbol,
        Some("MCU_ST_STM32F1xx:STM32F103C8Tx".to_string())
    );
...
"#;
    ...
    let model_id = db
        .import_simulation_model(comp_id, "spice", Some("behavioral"), spice, "spice3")
        .unwrap();
    assert!(model_id > 0);
...
fn test_import_upsert() {
    ...
    assert_eq!(
        comp.description,
        Some("Updated: 100nF 16V 0402".to_string())
    );
}
