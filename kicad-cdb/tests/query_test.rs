...
fn seed_db() -> ComponentDb {
    ...
    for (mpn, value_nf, voltage, pkg) in [
        ...
        let id = db
            .insert_component(&Component {
                ...
                category_id: cat_cap,
                description: Some(format!("{} {}V {}", value_nf, voltage, pkg)),
        ...
        db.insert_parameter(&Parameter {
            id: None,
            component_id: id,
            name: "voltage_rating".to_string(),
            value_numeric: Some(voltage),
            value_text: None,
    ...
    let mcu_id = db
        .insert_component(&Component {
            ...
            category_id: cat_mcu,
            description: Some("ARM Cortex-M3 72MHz 64KB Flash".to_string()),
    ...
    db.insert_parameter(&Parameter {
        id: None,
        component_id: mcu_id,
        name: "clock_max".to_string(),
        value_numeric: Some(72e6),
        value_text: None,
    ...
    db.insert_supply_info(&SupplyInfo {
        ...
        sku: Some("C15195".to_string()),
        price_breaks: Some("[[100, 0.02]]".to_string()),
        stock: Some(50000),
...
fn test_query_by_category() {
    ...
    assert!(caps
        .iter()
        .all(|c| c.mpn.starts_with("GRM") || c.mpn.starts_with("CL")));
}
...
fn test_query_by_parameter_range() {
    ...
    let results = db
        .query_by_parameter_range("capacitance", Some(100e-9), None)
        .unwrap();
    assert!(
        results.len() >= 3,
        "Should find caps >= 100nF, got {}",
        results.len()
    );

    // Cross-filter: of those, which have voltage >= 50V
    let high_v: Vec<_> = results
        .iter()
        .filter(|c| {
            if let Ok(params) = db.get_parameters(c.id.unwrap()) {
                params
                    .iter()
                    .any(|p| p.name == "voltage_rating" && p.value_numeric >= Some(50.0))
            } else {
                false
            }
        })
    ...
    assert_eq!(
        high_v.len(),
        1,
        "Only GRM188 (100nF, 50V) matches both filters"
    );
}
...
fn test_query_by_parameter_exact() {
    let db = seed_db();
    let results = db
        .query_by_parameter_exact("Capacitor", "voltage_rating", 16.0)
        .unwrap();
