...
fn test_insert_pins() {
    ...
    assert_eq!(
        loaded[1]
            .alt_functions
            .as_ref()
            .map(|v| v.iter().map(|a| a.function.clone()).collect::<Vec<_>>()),
        Some(vec!["ADC0".to_string(), "TIM2_CH1".to_string()])
    );
}
...
fn test_insert_parameters() {
    ...
    let params = vec![
        Parameter {
            ...
            value_numeric: Some(2.0),
            value_text: None,
            unit: Some("V".to_string()),
            typical: false,
            condition: None,
            source_page: Some("p.12".to_string()),
        },
        Parameter {
            id: None,
            component_id: comp_id,
            name: "vcc_max".to_string(),
            value_numeric: Some(3.6),
            value_text: None,
            unit: Some("V".to_string()),
            typical: false,
            condition: None,
    ...
    assert_eq!(
        interfaces.value_text,
        Some("SPI, I2C, USART, USB, CAN".to_string())
    );
}
...
"#;
    ...
    let id = db
        .insert_simulation_model(&SimulationModel {
            id: None,
            component_id: comp_id,
            ...
            verified: false,
            source: Some("manufacturer".to_string()),
            notes: None,
        })
        .unwrap();
    assert!(id > 0);
...
fn test_insert_design_rule() {
    ...
    let rule_id = db
        .insert_design_rule(&DesignRule {
            ...
            condition_expr: None,
            formula_expr: Some(
                "l_min = (vout * (1 - vout / vin)) / (fsw * 0.3 * iout)".to_string(),
            ),
...
// ===== Pin Alt Functions Structured Tests =====

#[test]
fn test_structured_alt_functions() {
    let db = setup_db();
    let comp_id = db.insert_component(&make_component(1)).unwrap();
    ...
    let pins = vec![Pin {
        ...
        alt_functions: Some(vec![
            PinAltFunction {
                function: "ADC0".to_string(),
                peripheral: Some("ADC1".to_string()),
                af_num: Some(0),
                signal_type: Some("analog".to_string()),
            },
            PinAltFunction {
                function: "TIM2_CH1".to_string(),
                peripheral: Some("TIM2".to_string()),
                af_num: Some(1),
                signal_type: Some("pwm".to_string()),
            },
        ]),
        description: None,
    }];
    ...
    let af = loaded[0].alt_functions.as_ref().unwrap();
    assert_eq!(af.len(), 2);
    assert_eq!(af[0].function, "ADC0");
    assert_eq!(af[0].peripheral, Some("ADC1".to_string()));
    assert_eq!(af[0].af_num, Some(0));
    assert_eq!(af[0].signal_type, Some("analog".to_string()));
    assert_eq!(af[1].function, "TIM2_CH1");
    assert_eq!(af[1].af_num, Some(1));
}

#[test]
fn test_backward_compat_simple_alt_functions() {
    let db = setup_db();
    let comp_id = db.insert_component(&make_component(1)).unwrap();

    // Insert with simple string format (old format)
    let pins = vec![Pin {
        id: None,
        component_id: comp_id,
        pin_number: "44".to_string(),
    ...
    assert_eq!(af.len(), 2);
    assert_eq!(af[0].function, "ADC0");
    assert_eq!(af[0].peripheral, None);
    assert_eq!(af[0].af_num, None);
}
...
fn test_lifecycle_update() {
    ...
    assert_eq!(comp.lifecycle, "active");

    assert!(db.update_lifecycle(comp_id, "nrnd").unwrap());
    let updated = db.get_component(comp_id).unwrap().unwrap();
    assert_eq!(updated.lifecycle, "nrnd");
}

#[test]
fn test_lifecycle_query() {
    let db = setup_db();
    let c1 = db.insert_component(&make_component(1)).unwrap();
    let mut c2 = make_component(1);
    c2.mpn = "STM32F407VGT6".to_string();
    let c2_id = db.insert_component(&c2).unwrap();

    db.update_lifecycle(c1, "obsolete").unwrap();
    db.update_lifecycle(c2_id, "active").unwrap();

    let active = db.query_by_lifecycle("active").unwrap();
    assert!(active.iter().any(|c| c.mpn == "STM32F407VGT6"));

    let obsolete = db.query_by_lifecycle("obsolete").unwrap();
    assert!(obsolete.iter().any(|c| c.mpn == "STM32F103C8T6"));
}

#[test]
fn test_lifecycle_constants() {
    assert!(lifecycle::is_valid("active"));
    assert!(lifecycle::is_valid("nrnd"));
    assert!(lifecycle::is_valid("obsolete"));
    assert!(lifecycle::is_valid("eol"));
    assert!(!lifecycle::is_valid("unknown"));
}

// ===== Simulation Model Management Tests =====

#[test]
fn test_list_all_simulation_models() {
    let db = setup_db();
    let c1 = db.insert_component(&make_component(1)).unwrap();
    let mut c2 = make_component(1);
    c2.mpn = "LM358".to_string();
    let c2_id = db.insert_component(&c2).unwrap();
    ...
    db.insert_simulation_model(&SimulationModel {
        id: None,
        component_id: c2_id,
        model_type: "ibis".to_string(),
        model_subcategory: None,
        model_text: "* IBIS model".to_string(),
        format: Some("ibis_v2.1".to_string()),
        port_mapping: None,
        verified: true,
        source: None,
        notes: None,
    ...
    let all = db.list_all_simulation_models(None, false).unwrap();
    assert_eq!(all.len(), 2);

    let spice_only = db.list_all_simulation_models(Some("spice"), false).unwrap();
    assert_eq!(spice_only.len(), 1);
    assert_eq!(spice_only[0].1, "STM32F103C8T6");

    let unverified = db.list_all_simulation_models(None, true).unwrap();
    assert_eq!(unverified.len(), 1);
    assert_eq!(unverified[0].1, "STM32F103C8T6");
}

#[test]
fn test_verify_simulation_model() {
    let db = setup_db();
    let comp_id = db.insert_component(&make_component(1)).unwrap();
    ...
    let models = db.get_simulation_models(comp_id).unwrap();
    assert!(!models[0].verified);

    assert!(db.verify_simulation_model(model_id).unwrap());
    let models = db.get_simulation_models(comp_id).unwrap();
    assert!(models[0].verified);
}
