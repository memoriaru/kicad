...
> **已知限制（层次原理图）：** `--power-flags` 仅对无子图的单文件（flat）原理图生效。包含 `sheets` 的层次原理图中，同一 global power net 上的多个 PWR_FLAG 会触发 KiCad ERC `pin_to_pin` 冲突。层次原理图的子图需手动在 KiCad 中添加 PWR_FLAG，或通过 power symbol（如 VCC/GND symbol，自带 `power_out` 引脚）隐式驱动电源网络。
...
│   └── error.rs
├── tests/                  # 集成测试
│   ├── fixtures/           # 测试 fixture（本工具链自产，可再分发）
│   ├── board_roundtrip.rs  # PCB IR / JSON5 / sexpr 往返
│   ├── roundtrip.rs        # PCB 元素计数回归
│   ├── sch_roundtrip.rs    # 原理图解析与往返
│   └── layout_test.rs      # 板级自动布局
└── CHANGELOG.md
```
