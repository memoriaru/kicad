...
impl SexprGenerator {
    pub(super) fn generate_symbol_instance(
        &mut self,
        output: &mut String,
        ...
        self.write_line(
            output,
            &format!("(lib_id \"{}\")", Self::normalize_lib_id(&component.lib_id)),
        );
        ...
        self.write_line(
            ...
            &format!(
                "(exclude_from_sim {})",
                Self::format_bool(component.exclude_from_sim)
            ),
        );
        self.write_line(
            output,
            &format!("(in_bom {})", Self::format_bool(component.in_bom)),
        );
        self.write_line(
            output,
            &format!("(on_board {})", Self::format_bool(component.on_board)),
        );
        self.write_line(
            output,
            &format!("(dnp {})", Self::format_bool(component.dnp)),
        );

        // fields_autoplaced
        if component.fields_autoplaced {
            self.write_line(output, "(fields_autoplaced yes)");
        }
        ...
        let needs_offset = pin_count > 2
            && component
                .properties_ext
                .iter()
                .any(|p| !p.hide && p.position == (0.0, 0.0, 0.0));
        let body_half_h = if needs_offset {
            let short = component
                .lib_id
                .split(':')
                .next_back()
                .unwrap_or(&component.lib_id);
            let spacing = 5.08_f64;
            // IC: pins split left/right, body height based on max pins per side
            // Connector: pins in single column (dual-row splits)
            let is_dual_connector =
                short.contains("02x") || short.contains("_02x") || short.contains("Conn_02");
            let max_per_side = if is_dual_connector {
                pin_count.div_ceil(2)
            } else {
                // IC layout: left half + right half
        ...
        // Footprint property (if set and not already in properties_ext)
        if let Some(fp) = &component.footprint {
            if !component
                .properties_ext
                .iter()
                .any(|p| p.name == "Footprint")
            {
                let fp_prop = crate::ir::Property {
                    name: "Footprint".into(),
                    value: fp.clone(),
                    position: (0.0, body_half_h + 5.08, 0.0),
                    hide: true,
                    effects: crate::ir::TextEffects::default(),
                    show_name: false,
                    do_not_autoplace: false,
                };
                self.generate_property(output, &fp_prop);
            }
        }
    ...
    pub(super) fn generate_property(&mut self, output: &mut String, prop: &crate::ir::Property) {
        ...
        if is_v9_plus {
            self.write_line(
                output,
                &format!("(show_name {})", Self::format_bool(prop.show_name)),
            );
            self.write_line(
                output,
                &format!(
                    "(do_not_autoplace {})",
                    Self::format_bool(prop.do_not_autoplace)
                ),
            );
        }
    ...
    pub(super) fn generate_instances(&mut self, output: &mut String, instances: &Instances) {
        ...
        for project in &instances.projects {
            self.write_line(
                output,
                &format!("(project \"{}\"", Self::escape_string(&project.name)),
            );
            self.indent_level += 1;
            ...
            for path in &project.paths {
                ...
                self.write_line(
                    output,
                    &format!("(reference \"{}\")", Self::escape_string(&path.reference)),
                );
    ...
    pub(super) fn generate_junction(&mut self, output: &mut String, junction: &Junction) {
        ...
        if junction.diameter != 0.0 {
            self.write_line(
                output,
                &format!("(diameter {})", Self::format_number(junction.diameter)),
            );
        }
    ...
    pub(super) fn generate_bus_entry(&mut self, output: &mut String, entry: &BusEntry) {
        ...
        self.write_line(
            output,
            &Self::format_at(entry.position.0, entry.position.1, 0.0),
        );
        self.write_line(
            output,
            &format!(
                "(size {} {})",
                Self::format_number(entry.size.0),
                Self::format_number(entry.size.1)
            ),
        );
    ...
    pub(super) fn distribute_sheet_pins(sheet: &Sheet) -> Vec<SheetPin> {
        let all_at_origin = sheet
            .pins
            .iter()
            .all(|p| p.position.0 == 0.0 && p.position.1 == 0.0);
        ...
        let (w, h) = sheet.size;
        let left_pins: Vec<&SheetPin> = sheet
            .pins
            .iter()
            .filter(|p| matches!(p.pin_type, crate::ir::PinType::Input))
            .collect();
        let right_pins: Vec<&SheetPin> = sheet
            .pins
            .iter()
            .filter(|p| matches!(p.pin_type, crate::ir::PinType::Output))
            .collect();
        let other_pins: Vec<&SheetPin> = sheet
            .pins
            .iter()
            .filter(|p| {
                !matches!(
                    p.pin_type,
                    crate::ir::PinType::Input | crate::ir::PinType::Output
                )
            })
            .collect();
