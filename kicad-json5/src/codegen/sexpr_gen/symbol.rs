...
impl SexprGenerator {
    pub(super) fn generate_lib_symbols(
        &mut self,
        output: &mut String,
        symbols: &[Symbol],
        nets: &[Net],
    ...
    fn generate_symbol_def(&mut self, output: &mut String, symbol: &Symbol) {
        ...
        // Round-trip fidelity: when the source schematic carried its own pin graphics,
        // prefer them over the embedded template — auto-generated geometry would
        // silently re-place pins and mis-attach global labels.
        if matches!(self.effective_version, KicadVersion::V9 | KicadVersion::V10)
            && symbol.graphics.is_empty()
        {
            if let Some(sexpr) =
        ...
        self.write_line(
            output,
            &format!("(symbol \"{}\"", Self::normalize_lib_id(&symbol.lib_id)),
        );
        self.indent_level += 1;
        ...
        let should_hide_pin_numbers =
            symbol.pin_numbers_hidden || (symbol.pins.len() == 2 && Self::is_passive_2pin(symbol));
        ...
        if symbol.pin_names_hidden || symbol.pin_name_offset != 0.254 {
            if symbol.pin_names_hidden {
                if is_v9_plus {
                    ...
                    self.write_line(
                        output,
                        &format!("(offset {})", Self::format_number(symbol.pin_name_offset)),
                    );
                    self.write_line(output, "(hide yes)");
                    self.indent_level -= 1;
                    self.write_line(output, ")");
        ...
        self.write_line(
            ...
            &format!(
                "(exclude_from_sim {})",
                Self::format_bool(symbol.exclude_from_sim)
            ),
        );
        ...
        self.write_line(
            output,
            &format!("(in_bom {})", Self::format_bool(symbol.in_bom)),
        );
        self.write_line(
            output,
            &format!("(on_board {})", Self::format_bool(symbol.on_board)),
        );
        ...
        if matches!(self.effective_version, KicadVersion::V10) {
            self.write_line(
                output,
                &format!("(in_pos_files {})", Self::format_bool(symbol.in_pos_files)),
            );
            self.write_line(
                output,
                &format!(
                    "(duplicate_pin_numbers_are_jumpers {})",
                    Self::format_bool(symbol.duplicate_pin_numbers_are_jumpers)
                ),
            );
        }
        ...
        if symbol.units.is_empty() {
            if !symbol.graphics.is_empty() {
                // KiCad lib_symbols 要求子符号名带父名前缀（如 "C_0_1"），且 pin
                // 必须位于 unit != 0 的子符号（_1_1）——unit 0 只放公共图形。
                // power 符号例外：图形与 pin 同在 _0_1。
                let short = symbol
                    .lib_id
                    .split(':')
                    .next_back()
                    .unwrap_or(&symbol.lib_id);
                let is_pin = |g: &GraphicElement| matches!(g, GraphicElement::Pin(_));
                let has_pins = symbol.graphics.iter().any(is_pin);
                let has_graphics = symbol.graphics.iter().any(|g| !is_pin(g));
                let groups: Vec<(String, Vec<&GraphicElement>)> =
                    if symbol.is_power || (has_pins && !has_graphics) {
                        vec![(format!("{}_0_1", short), symbol.graphics.iter().collect())]
                    } else if has_pins && has_graphics {
                        vec![
                            (
                                format!("{}_0_1", short),
                                symbol.graphics.iter().filter(|g| !is_pin(g)).collect(),
                            ),
                            (
                                format!("{}_1_1", short),
                                symbol.graphics.iter().filter(|g| is_pin(g)).collect(),
                ...
                for (name, graphics) in groups {
                    self.write_line(output, &format!("(symbol \"{}\"", name));
                    self.indent_level += 1;
                ...
                || Self::is_passive_2pin(symbol)
                || Self::detect_default_kind(symbol) != DefaultSymbolKind::TwoPin
            {
    ...
    fn generate_default_symbol_units(&mut self, output: &mut String, symbol: &Symbol) {
        ...
        match kind {
            DefaultSymbolKind::Connector => self.gen_connector_unit(output, symbol, base),
            DefaultSymbolKind::Ic => self.gen_ic_unit(output, symbol, base),
            DefaultSymbolKind::Resistor => self.gen_resistor_unit(output, symbol, base),
            DefaultSymbolKind::Capacitor => self.gen_capacitor_unit(output, symbol, base),
            DefaultSymbolKind::Inductor => self.gen_inductor_unit(output, symbol, base),
            DefaultSymbolKind::Diode | DefaultSymbolKind::Led => {
                self.gen_diode_unit(output, symbol, base, matches!(kind, DefaultSymbolKind::Led))
            }
    ...
    pub(super) fn get_pin_or_default(symbol: &Symbol, index: usize, default_number: &str) -> Pin {
        symbol.pins.get(index).cloned().unwrap_or_else(|| Pin {
    ...
    fn gen_default_pin(&mut self, output: &mut String, pin: &Pin, x: f64, y: f64, rotation: f64) {
        ...
        self.write_line(
            output,
            &format!("(name \"{}\"", Self::escape_string(&pin.name)),
        );
        self.indent_level += 1;
        self.write_line(output, "(effects (font (size 1.27 1.27))))");
        self.indent_level -= 1;
    ...
    fn gen_connector_unit(&mut self, output: &mut String, symbol: &Symbol, base: &str) {
    ...
    fn gen_ic_unit(&mut self, output: &mut String, symbol: &Symbol, base: &str) {
        let pin_count = symbol.pins.len();
        let spacing = 5.08;
        let pin_length = 2.54;

        // >32 pins: four-side layout (matches compute_pin_positions in
        // auto.rs) — two columns would tower ~100mm and put pins off-sheet,
        // where KiCad silently drops them from the netlist.
        if pin_count > 32 && pin_count.is_multiple_of(4) {
            ...
            self.indent_level += 1;
            for (i, pin) in symbol.pins.iter().enumerate() {
                let (x, y, rot) = if i < per_side {
                    (
                        -(body_half + pin_length),
                        half_span - i as f64 * spacing,
                        0.0,
                    )
                } else if i < 2 * per_side {
                    let j = i - per_side;
                    (
                        -half_span + j as f64 * spacing,
                        -(body_half + pin_length),
                        90.0,
                    )
                ...
                self.gen_default_pin(output, pin, x, y, rot);
            }
            self.indent_level -= 1;
            self.write_line(output, ")");
            return;
        ...
        for (i, pin) in symbol.pins.iter().enumerate() {
            if i < left_count {
                ...
                self.gen_default_pin(output, pin, -body_hw - pin_length, y, 0.0);
            }
        }

        for (i, pin) in symbol.pins.iter().skip(left_count).enumerate() {
            let y = ((right_count - 1) as f64 / 2.0 - i as f64) * spacing;
            self.gen_default_pin(output, pin, body_hw + pin_length, y, 180.0);
        }
    ...
    fn gen_resistor_unit(&mut self, output: &mut String, symbol: &Symbol, base: &str) {
        // _0_1: IEC-style resistor body
        self.write_line(output, &format!("(symbol \"{}_0_1\"", base));
        self.indent_level += 1;
        self.write_line(output, "(rectangle");
        self.indent_level += 1;
        self.write_line(output, "(start -1.016 -2.54) (end 1.016 2.54)");
        self.gen_default_stroke(output, 0.254);
        self.write_line(output, "(fill (type none))");
        self.indent_level -= 1;
        self.write_line(output, ")");
        ...
        self.gen_kicad_pin(output, "passive", &pin1.number, "", 0.0, 5.81, 270.0, 3.27);
        let pin2 = Self::get_pin_or_default(symbol, 1, "2");
        self.gen_kicad_pin(output, "passive", &pin2.number, "", 0.0, -5.81, 90.0, 3.27);
        self.indent_level -= 1;
        self.write_line(output, ")");
    }

    /// Capacitor template: two parallel plates + 2 vertical pins
    fn gen_capacitor_unit(&mut self, output: &mut String, symbol: &Symbol, base: &str) {
        // _0_1: two plates
        self.write_line(output, &format!("(symbol \"{}_0_1\"", base));
        self.indent_level += 1;
    ...
    fn gen_inductor_unit(&mut self, output: &mut String, symbol: &Symbol, base: &str) {
    ...
    fn gen_diode_unit(&mut self, output: &mut String, symbol: &Symbol, base: &str, is_led: bool) {
    ...
    fn gen_twopin_unit(&mut self, output: &mut String, symbol: &Symbol, base: &str) {
        // _0_1: rectangle body
        self.write_line(output, &format!("(symbol \"{}_0_1\"", base));
        self.indent_level += 1;
        self.write_line(output, "(rectangle");
