...
pub use codegen::{
    BoardSexprConfig, BoardSexprGenerator, Json5Config, Json5Generator, KicadVersion, SexprConfig,
    SexprGenerator,
};
...
pub fn generate_board_json5(board: &ir::Board) -> Result<String> {
    ...
    if let Some(ref tb) = board.title_block {
        ...
        if let Some(ref v) = tb.title {
            s.push_str(&format!("    title: \"{}\",\n", escape_json5(v)));
        }
        if let Some(ref v) = tb.date {
            s.push_str(&format!("    date: \"{}\",\n", v));
        }
        if let Some(ref v) = tb.rev {
            s.push_str(&format!("    rev: \"{}\",\n", v));
        }
        if let Some(ref v) = tb.company {
            s.push_str(&format!("    company: \"{}\",\n", escape_json5(v)));
        }
        s.push_str("  },\n");
    }
        ...
        for fp in &board.footprints {
            ...
            if let Some(ref attr) = fp.attr {
                let attr_str = match attr {
                    ...
                    ir::board::FootprintAttr::Other(vals) => format!(
                        "[{}]",
                        vals.iter()
                            .map(|v| format!("\"{}\"", v))
                            .collect::<Vec<_>>()
            ...
            if !fp.properties_ext.is_empty() {
                ...
                for prop in &fp.properties_ext {
                    ...
                    s.push_str(&format!("          layer: \"{}\",\n", prop.layer));
                    if prop.hide {
                        s.push_str("          hide: true,\n");
                    }
                    if prop.unlocked {
                        s.push_str("          unlocked: true,\n");
                    }
                    s.push_str(&format!(
                        "          effects: {{ font_size: [{}, {}], thickness: {}{}{} }},\n",
                        prop.effects.font_size.0,
                        prop.effects.font_size.1,
                        prop.effects.font_thickness,
                        if prop.effects.bold {
                            ", bold: true"
                        } else {
                            ""
                        },
    ...
    if !board.graphics.is_empty() {
        ...
        for g in &board.graphics {
            match &g.kind {
                ...
                ir::board::BoardGraphicKind::Text {
                    text,
                    position,
                    font_size,
                } => {
...
mod tests {
    ...
    fn test_roundtrip_full_graphics() {
        ...
        let json5 = generate_board_json5(&board).unwrap();
        assert!(
            json5.contains("fp_lines"),
            "json5 export must emit fp_lines"
        );
        assert!(
            json5.contains("fp_rects"),
            "json5 export must emit fp_rects"
        );
        ...
        assert!(sexpr.matches("fp_line").count() >= 1);
        assert!(sexpr.matches("fp_rect").count() >= 1);
        let final_board = parse_board(&sexpr).unwrap();
        assert_eq!(
            final_board.footprints[0].fp_lines.len(),
            1,
            "fp_lines lost in sexpr hop"
        );
        assert_eq!(
            final_board.footprints[0].fp_circles.len(),
            1,
            "fp_circles lost in sexpr hop"
        );
        assert_eq!(
            final_board.footprints[0].fp_arcs.len(),
            1,
            "fp_arcs lost in sexpr hop"
        );
        assert_eq!(
            final_board.footprints[0].fp_rects.len(),
            1,
            "fp_rects lost in sexpr hop"
        );
        assert_eq!(
            final_board.footprints[0].fp_polys.len(),
            1,
            "fp_polys lost in sexpr hop"
        );
        // legacy synthesis always adds reference/value fp_texts on top of ours (P2-4)
        assert!(
            final_board.footprints[0]
                .fp_texts
                .iter()
        ...
        assert_eq!(
            final_board.graphics.len(),
            3,
            "board graphics lost in sexpr hop"
        );
    }
