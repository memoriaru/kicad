...
fn value_to_schematic(value: &Value) -> Result<Schematic> {
    ...
    schematic.metadata.generator =
        get_str(obj, "generator").unwrap_or_else(|| "kicad-json5".into());
    ...
    if let Some(nets) = obj.get("nets").and_then(|v| v.as_array()) {
        ...
        eprintln!(
            "[json5_parser] WARNING: 'nets' key not found or not an array. Top-level keys: {:?}",
    ...
    if let Some(comps) = obj.get("components").and_then(|v| v.as_array()) {
        for cv in comps {
            schematic.components.push(value_to_component(cv, &net_map));
        }
    }
...
fn value_to_symbol(value: &Value) -> Symbol {
    ...
    if let Some(o) = obj {
        ...
        symbol.exclude_from_sim = o
            .get("exclude_from_sim")
            .and_then(|v| v.as_bool())
            .unwrap_or(false);
        symbol.in_pos_files = o
            .get("in_pos_files")
            .and_then(|v| v.as_bool())
            .unwrap_or(true);
        symbol.duplicate_pin_numbers_are_jumpers = o
            .get("duplicate_pin_numbers_are_jumpers")
            .and_then(|v| v.as_bool())
            .unwrap_or(false);
        ...
        // pin graphics (original geometry). Round-trip fidelity: sexpr_gen writes
        // these positions instead of falling back to auto-layout.
        if let Some(graphics) = o.get("graphics").and_then(|v| v.as_array()) {
            for gv in graphics {
                let go = gv.as_object();
                if go.and_then(|o| o.get("type")).and_then(|v| v.as_str()) != Some("pin") {
                    continue;
                }
                let number = go.and_then(|o| get_str(o, "number")).unwrap_or_default();
                let name = go.and_then(|o| get_str(o, "name")).unwrap_or_default();
                let mut pg = crate::ir::PinGraphic::new(number, name);
                if let Some(t) = go.and_then(|o| o.get("pin_type")).and_then(|v| v.as_str()) {
                    pg.pin_type = PinType::from_str(t);
                }
                if let Some(t) = go.and_then(|o| o.get("shape")).and_then(|v| v.as_str()) {
                    pg.shape = crate::ir::PinShape::from_str(t);
                }
                if let Some(at) = go.and_then(|o| o.get("at")).and_then(|v| v.as_array()) {
                    let xs: Vec<Option<f64>> = at.iter().map(|v| v.as_f64()).collect();
                    if xs.len() == 3 {
                        pg.position = (
                            xs[0].unwrap_or(0.0),
                            xs[1].unwrap_or(0.0),
                ...
                if let Some(l) = go.and_then(|o| o.get("length")).and_then(|v| v.as_f64()) {
                    pg.length = l;
                }
                symbol.graphics.push(crate::ir::GraphicElement::Pin(pg));
            }
        }
    }
...
fn value_to_component(value: &Value, net_map: &HashMap<u32, String>) -> SymbolInstance {
    let o = value.as_object();
    let lib_id = o.and_then(|o| get_str(o, "lib_id")).unwrap_or_default();
    let reference = o.and_then(|o| get_str(o, "ref")).unwrap_or_default();
    ...
    if let Some(o) = o {
        ...
        comp.unit = o.get("unit").and_then(|v| v.as_u64()).unwrap_or(1) as u32;

        // KiCad 8+ flags
        comp.exclude_from_sim = o
            .get("exclude_from_sim")
            .and_then(|v| v.as_bool())
            .unwrap_or(false);
                        ...
                        for path_val in paths {
                            if let Some(path_obj) = path_val.as_object() {
                                project.paths.push(InstancePath {
                                    ...
                                    unit: path_obj.get("unit").and_then(|v| v.as_u64()).unwrap_or(1)
                                        as u32,
                                });
                            }
                        }
        ...
        // pins: support both array format [{number, name, type, net}, ...] and object format {num: {name, type, net}, ...}
        if let Some(pins_val) = o.get("pins") {
            if let Some(pins_arr) = pins_val.as_array() {
                // Array format: each element has number, name, type, net fields
                for pv in pins_arr {
                    let po = pv.as_object();
                    let number = po.and_then(|o| get_str(o, "number")).unwrap_or_default();
                    let net_id = po
                    ...
                    let net_name = net_id
                        ...
                        .cloned()
                        .or_else(|| po.and_then(|o| get_str(o, "net_name")));
            ...
            } else if let Some(pins_obj) = pins_val.as_object() {
                // Object format: keyed by pin number
                for (num, pv) in pins_obj {
                    let po = pv.as_object();
                    let net_id = po
                        .and_then(|o| o.get("net"))
                        .and_then(|v| v.as_u64())
...
fn value_to_label(value: &Value) -> Label {
    let o = value.as_object();
    let text = o.and_then(|o| get_str(o, "text")).unwrap_or_default();
    let label_type = o
        .and_then(|o| get_str(o, "type"))
...
mod tests {
    ...
    fn test_parse_metadata() {
        ...
        assert_eq!(
            sch.metadata.title_block.title,
            Some("Test Schematic".into())
        );
