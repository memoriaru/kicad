...
impl HierarchicalProject {
    ...
    pub fn load_from_file(root_path: &Path) -> crate::Result<Self> {
        let root_dir = root_path
            ...
            .to_path_buf();
        let source = std::fs::read_to_string(root_path).map_err(|e| {
            crate::Error::InvalidSExpr(format!(
                "Failed to read root schematic {}: {}",
                root_path.display(),
                e
            ))
        })?;
...
pub fn check_sheet_interfaces(project: &HierarchicalProject) -> InterfaceCheckResult {
    ...
    for sub in project.sub_sheets.values() {
        if sub.sheets.is_empty() {
            continue;
        }
...
fn check_one_level(
    ...
    for sheet in &parent.sheets {
        ...
        let child = match sub_sheets.get(&sheet_file) {
            ...
            None => {
                findings.push(InterfaceFinding {
                    ...
                    pin_name: "—".into(),
                    message: format!(
                        "Sub-sheet file '{}' not found or failed to parse",
                        sheet_file
                    ),
                });
                continue;
        ...
        let child_labels: HashMap<&str, &Label> = child
            .labels
            .iter()
        ...
        let parent_pin_names: HashSet<&str> = sheet.pins.iter().map(|p| p.name.as_str()).collect();
        for label_name in child_labels.keys() {
...
{}
    ...
    fn test_interface_pin_label_match() {
        ...
        let child = make_sch(r#"  (hierarchical_label "DATA" (shape input) (at 100 100))"#);
    ...
    fn test_interface_missing_label() {
        ...
        let child = make_sch(r#"  (hierarchical_label "OTHER" (shape output) (at 100 100))"#);
    ...
    (pin "CLK" input (at 50 55))
        ...
        assert!(result.findings.iter().any(|f| f.severity == "error"
            && f.pin_name == "CLK"
            && f.message.contains("no matching")));
    }
  ...
  (hierarchical_label "EXTRA" (shape output) (at 100 120))"#,
    ...
    (pin "DATA" input (at 50 55))
        ...
        assert!(result
            .findings
            .iter()
            .any(|f| f.severity == "warning" && f.pin_name == "EXTRA"));
    }
}
