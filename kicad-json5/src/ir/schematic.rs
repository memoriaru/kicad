...
use super::{
    Bus, BusEntry, Junction, Label, Net, NoConnect, Property, RenderHint, Symbol, SymbolInstance,
    Wire,
};
...
impl Schematic {
                ...
                match key {
                    "pin_numbers" => {
                        ...
                        symbol.pin_numbers_hidden = sub_list.iter().any(|s| {
                            s.is_ident("hide") ||
                            (s.is_list() && s.as_list().unwrap().first().is_some_and(|f| f.is_ident("hide")))
                        });
                    }
                    "pin_names" => {
                        ...
                        for sub_item in &sub_list[1..] {
                            if let SExpr::List(offset_list) = sub_item {
                                if offset_list.first().is_some_and(|f| f.is_ident("offset")) {
                    ...
                    "symbol" => {
                        // Sub-symbol block (e.g. "FPC_30pin_1_1"): hoist its pins and
                        // graphics into the parent symbol so that reverse output keeps
                        // the original pin geometry. Without this, pins vanish and the
                        // writer falls back to auto-layout, silently re-positioning pins
                        // and mis-attaching global labels on round-trip.
                        if let Some(sub) = Self::parse_symbol_def(sub_list) {
                            for g in sub.graphics {
                                symbol.graphics.push(g);
                            }
                        }
                    }
                    "property"
                    ...
                    #[allow(unreachable_patterns)] // shadowed by the hoist arm above; kept pending semantic review
                    "symbol"
    ...
    fn parse_symbol_instance(
        ...
        for item in &list[1..] {
            if let SExpr::List(sub_list) = item {
                if sub_list.first().is_some_and(|f| f.is_ident("lib_id")) {
        ...
        for item in &list[2..] {
            if let SExpr::List(sub_list) = item {
                ...
                match key {
                    ...
                    "exclude_from_sim" => {
                        component.exclude_from_sim =
                            sub_list.get(1).and_then(get_bool).unwrap_or(false);
                    }
                    "in_bom" => {
                        ...
                        for prop_item in &sub_list[3..] {
                            if let SExpr::List(prop_sub) = prop_item {
                                ...
                                match get_ident(&prop_sub[0]) {
                                    ...
                                    Some("show_name") => {
                                        prop.show_name =
                                            prop_sub.get(1).and_then(get_bool).unwrap_or(false);
                                    }
                                    Some("do_not_autoplace") => {
                                        prop.do_not_autoplace =
                                            prop_sub.get(1).and_then(get_bool).unwrap_or(false);
                                    }
                                    _ => {}
                                }
                    ...
                    "instances" => {
                        for inst_item in &sub_list[1..] {
                            if let SExpr::List(proj_list) = inst_item {
                                if proj_list.first().is_some_and(|f| f.is_ident("project")) {
                                    let proj_name =
                                        proj_list.get(1).and_then(get_string).unwrap_or_default();
                                    ...
                                    for path_item in &proj_list[2..] {
                                        if let SExpr::List(path_list) = path_item {
                                            if path_list.first().is_some_and(|f| f.is_ident("path"))
                                            {
                                                let path_str = path_list
                                                    .get(1)
                                                    .and_then(get_string)
                                                    .unwrap_or_default();
    ...
    fn parse_pin_instance(
        list: &[SExpr],
        net_map: &HashMap<u32, String>,
    ) -> Option<super::PinInstance> {
                        ...
                        for stroke_item in &sub_list[1..] {
                            if let SExpr::List(stroke_list) = stroke_item {
                                if stroke_list[0].is_ident("width") {
                                    wire.stroke.width =
                                        stroke_list.get(1).and_then(get_number).unwrap_or(0.0);
                                } else if stroke_list[0].is_ident("type") {
                                    wire.stroke.stroke_type = stroke_list
                                        .get(1)
                                        .and_then(get_ident)
