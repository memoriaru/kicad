...
fn load_from(dir: &Path, path: &str) -> String {
    let p = dir.join(path);
    assert!(
        p.exists(),
...
fn roundtrip_pcb(source: &str) {
    ...
    let fp_line_count = board
        .footprints
        .iter()
        .map(|f| f.fp_lines.len())
        .sum::<usize>();
    let fp_arc_count = board
        .footprints
        .iter()
        .map(|f| f.fp_arcs.len())
        .sum::<usize>();
    let fp_circle_count = board
        .footprints
        .iter()
        .map(|f| f.fp_circles.len())
        .sum::<usize>();
    let fp_rect_count = board
        .footprints
        .iter()
        .map(|f| f.fp_rects.len())
        .sum::<usize>();
    let fp_poly_count = board
        .footprints
        .iter()
        .map(|f| f.fp_polys.len())
        .sum::<usize>();
    let pad_count = board.footprints.iter().map(|f| f.pads.len()).sum::<usize>();
    let filled_poly_count = board
        .zones
        .iter()
        .map(|z| z.filled_polygons.len())
        .sum::<usize>();
    ...
    assert_eq!(
        output.matches("(footprint \"").count(),
        board.footprints.len(),
        "footprints"
    );
    assert_eq!(
        output.matches("(segment\n").count(),
        board.segments.len(),
        "segments"
    );
    ...
    assert_eq!(
        output.matches("(fp_line\n").count(),
        fp_line_count,
        "fp_lines"
    );
    assert_eq!(output.matches("(fp_arc\n").count(), fp_arc_count, "fp_arcs");
    assert_eq!(
        output.matches("(fp_circle\n").count(),
        fp_circle_count,
        "fp_circles"
    );
    assert_eq!(
        output.matches("(fp_rect\n").count(),
        fp_rect_count,
        "fp_rects"
    );
    assert_eq!(
        output.matches("(fp_poly\n").count(),
        fp_poly_count,
        "fp_polys"
    );
    assert_eq!(
        output.matches("(filled_polygon\n").count(),
        filled_poly_count,
        "filled_polygons"
    );
...
fn test_tiny_scarab_graphics_detail() {
    ...
    let poly_count = board
        .graphics
        .iter()
        .filter(|g| {
            matches!(
                &g.kind,
                kicad_json5::ir::board::BoardGraphicKind::Poly { .. }
            )
        })
        .count();
    ...
    assert_eq!(
        poly_count, 0,
        "Board-level gr_poly should be 0 (the ones in file are inside pad primitives)"
    );
}
