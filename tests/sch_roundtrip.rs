...
/// HQ EDA demo files (not redistributable). Override with KICAD_JSON5_HQ_DEMO_DIR.
...
fn parse_sch_from(dir: &PathBuf, name: &str) -> Schematic {
...
fn test_roundtrip_battery_power_board() {
    ...
    assert_eq!(original.components.len(), reparsed.components.len(), "component count");
    assert_eq!(original.lib_symbols.len(), reparsed.lib_symbols.len(), "lib_symbols count");
}

// ── HQ EDA demo schematics (opt-in, not redistributable) ────
// Run with: KICAD_JSON5_HQ_DEMO_DIR=/path/to/demo cargo test -- --ignored
