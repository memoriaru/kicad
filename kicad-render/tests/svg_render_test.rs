...
use kicad_render::painter::{
    Junction, JunctionPainter, Label, LabelPainter, LabelShape, LabelType, Painter, PinGraphic,
    PinPainter, PinType, WirePainter, WireSegment,
};
use kicad_render::{
    Arc, BoundingBox, Circle, Color, Fill, LayerElement, LayerElementType, LayerId, LayerSet,
    Matrix, Point, Polygon, Polyline, RenderContext, Renderer, Stroke, SvgRenderer,
};
use std::fs;
...
fn create_test_schematic_layers() -> LayerSet {
    ...
    let pin_painter = PinPainter::new(pin, Matrix::identity(), wire_color, Color::black());
    ...
    let circle = Circle::new(Point::new(75.0, 30.0), 5.0).with_fill(Fill::solid(Color::red()));
...
fn test_primitive_rendering() {
    ...
    // Test circle
    let circle = Circle::new(Point::new(25.0, 25.0), 10.0).with_fill(Fill::solid(Color::red()));
...
fn test_layer_z_order() {
    ...
    assert!(
        bg_z < fg_z,
        "Background should have lower z-index than foreground"
    );
...
fn test_transform_rendering() {
    ...
    let circle1 = Circle::new(Point::new(10.0, 10.0), 5.0).with_fill(Fill::solid(Color::black()));
    ...
    let circle2 = Circle::new(Point::new(10.0, 10.0), 5.0).with_fill(Fill::solid(Color::red()));
    renderer.draw_circle(&circle2);

    // Restore and draw again
    renderer.restore();
    let circle3 = Circle::new(Point::new(10.0, 30.0), 5.0).with_fill(Fill::solid(Color::blue()));
...
fn test_wire_painter() {
    ...
    assert!(
        bbox.min_y() <= 0.0,
        "min_y should be <= 0.0, got {}",
        bbox.min_y()
    );
    assert!(
        bbox.max_y() >= 50.0,
        "max_y should be >= 50.0, got {}",
        bbox.max_y()
    );
}
...
fn test_pin_painter() {
    ...
    let painter = PinPainter::new(pin, Matrix::identity(), Color::blue(), Color::black());
    painter.paint(&mut layers);
...
fn test_label_painter() {
    ...
    assert!(
        !label_layer.elements.is_empty(),
        "Label should have elements"
    );
}
