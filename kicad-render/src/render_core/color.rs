    ...
    fn from_hex(hex: &str) -> Self {
        ...
        let (r, g, b, a) = match hex.len() {
            3 => {
                // #RGB -> #RRGGBB
                let r =
                    u8::from_str_radix(&format!("{}{}", &hex[0..1], &hex[0..1]), 16).unwrap_or(0);
                let g =
                    u8::from_str_radix(&format!("{}{}", &hex[1..2], &hex[1..2]), 16).unwrap_or(0);
                let b =
                    u8::from_str_radix(&format!("{}{}", &hex[2..3], &hex[2..3]), 16).unwrap_or(0);
                (r, g, b, 255u8)
            }
            6 => {
                // #RRGGBB
