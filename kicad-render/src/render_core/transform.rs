...
impl Camera2 {
    ...
    pub fn bbox(&self) -> super::BoundingBox {
        let inv = self.matrix().inverse().unwrap_or_else(Matrix::identity);
    ...
    pub fn apply_to_canvas(&self, ctx: &web_sys::CanvasRenderingContext2d) {
        self.viewport_size
            .set(ctx.canvas.width() as f64, ctx.canvas.height() as f64);
...
mod tests {
    ...
    fn test_camera_matrix() {
        let camera = Camera2::new(Point::new(100.0, 100.0), Point::new(50.0, 50.0), 2.0);
