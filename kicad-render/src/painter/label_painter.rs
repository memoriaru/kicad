...
impl LabelPainter {
    ...
    /// Text is centered inside the flag shape with equal margins on both sides.
    /// Offset = (flag_length - text_render_width) / 2
    fn paint_label_text(&self, layers: &mut LayerSet) {
        let layer = layers
            .get_layer_mut(LayerId::Labels)
            .expect("Labels layer missing from LayerSet");

        // Flag length calculation (matches paint_global_shape)
        let margin = self.label.font_size * constants::LABEL_SIZE_RATIO;
        let flag_len = self.label.text.len() as f64 * self.label.font_size
            + 2.0 * margin
            + constants::LINE_WIDTH;

        // Actual rendered text width
        let text_w =
            self.label.text.len() as f64 * self.label.font_size * constants::CHAR_WIDTH_RATIO;

        // Center text inside flag, then subtract 1mm per 3 chars for line width compensation
        let pad = (flag_len - text_w) / 2.0 - self.label.text.len() as f64 / 6.0;

        let (pos, text_rot, text_anchor) = match self.label.rotation {
            0 => (
                Point::new(self.label.position.x + pad, self.label.position.y),
                0.0,
                "start",
            ),
            180 => (
            ...
            90 => (
                Point::new(self.label.position.x, self.label.position.y + pad),
                90.0,
                "start",
            ),
            270 => (
            ...
            _ => (self.label.position, 0.0, "start"),
        };
    ...
    fn paint_global_shape(&self, layer: &mut crate::layer::Layer, pos: Point, stroke: Stroke) {
        ...
        let polygon = Polygon::from_points(&world_pts).with_stroke(stroke);
    ...
    fn paint_hierarchical_shape(
        ...
        let pts: Vec<(f64, f64)> = match self.label.shape {
            ...
            LabelShape::Input => vec![(s, s2), (s2, s2), (0.0, 0.0), (s2, -s2), (s, -s2), (s, s2)],
            LabelShape::Bidirectional | LabelShape::TriState => {
                vec![(s2, s2), (s, 0.0), (s2, -s2), (0.0, 0.0), (s2, s2)]
            }
            LabelShape::Passive => vec![(0.0, s2), (s, s2), (s, -s2), (0.0, -s2), (0.0, s2)],
        };
