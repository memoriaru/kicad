...
fn orient_pin_label(
    ...
    h_align: &'static str,
    v_align: &'static str,
) -> (f64, f64, f64, &'static str, &'static str) {
    let (dx, dy, rot, anchor): (f64, f64, f64, &'static str) = match orientation {
        ...
        PinOrientation::Left => (
            ...
            oy,
            0.0,
            if h_align == "left" { "end" } else { h_align },
        ),
...
impl PinPainter {
    ...
    fn correct_vertical_anchor(
        offset_y: &mut f64,
        text_anchor: &mut &'static str,
        is_vertical: bool,
    ) {
        if is_vertical {
            *offset_y = -*offset_y;
    ...
    fn paint_pin_body_and_shape(&self, layers: &mut LayerSet) {
        let layer = layers
            .get_layer_mut(LayerId::SymbolPin)
        ...
        let clock_notch =
            |layer: &mut crate::layer::Layer, p0: Point, dir: Point, stroke: Stroke| {
                if dir.y == 0.0 {
    ...
    fn paint_pin_name(&self, layers: &mut LayerSet) {
        ...
        let (offset_x, mut offset_y, rotation, mut text_anchor, dominant_baseline) =
            if pin_name_offset > 0.0 {
