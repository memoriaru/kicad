...
use crate::render_core::graphics::{Fill, Polygon, Polyline, Stroke};
...
impl SheetPainter {
    ...
    fn paint_box(&self, layers: &mut LayerSet) {
        let layer = layers
            .get_layer_mut(LayerId::SheetBackground)
            .expect("SheetBackground layer missing from LayerSet");
        let (x, y) = (self.sheet.position.x, self.sheet.position.y);
        let (w, h) = self.sheet.size;

        let polygon =
            Polygon::from_points(&[(x, y), (x + w, y), (x + w, y + h), (x, y + h), (x, y)])
    ...
    fn paint_pins(&self, layers: &mut LayerSet) {
        let layer = layers
            .get_layer_mut(LayerId::SheetPin)
            .expect("SheetPin layer missing from LayerSet");
