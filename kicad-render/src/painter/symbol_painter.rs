...
use super::pin_painter::{PinGraphic, PinPainter};
use super::Painter;
use crate::bridge;
use crate::constants;
use crate::layer::{LayerElement, LayerElementType, LayerId, LayerSet};
use crate::render_core::graphics::{Arc as RcArc, Circle as RcCircle, Fill, Stroke};
...
impl SymbolPainter {
            ...
            for ge in &self.body_graphics {
                if let GraphicElement::Rectangle(ir_rect) = ge {
                    if ir_rect.fill.fill_type == kicad_json5::ir::FillType::Background {
                        let mut rc_poly = bridge::convert_rectangle_with_fill(
                            ir_rect,
                            self.outline_color,
                            self.outline_color,
                        );
            ...
            for ge in &self.body_graphics {
                match ge {
                    GraphicElement::Polyline(ir_poly) => {
                        let mut rc_poly =
                            bridge::convert_polyline_with_color(ir_poly, self.outline_color);
                        rc_poly.points = transform_points(&rc_poly.points, &transform);
                    ...
                    GraphicElement::Circle(ir_circle) => {
                        let rc_circle = bridge::convert_circle_with_fill(
                            ir_circle,
                            self.outline_color,
                            self.outline_color,
                        );
                        ...
                        let mut transformed =
                            RcCircle::new(center, rc_circle.radius).with_fill(rc_circle.fill);
                    ...
                    GraphicElement::Arc(ir_arc) => {
                        let start_p =
                            transform.transform(&Point::new(ir_arc.start.0, ir_arc.start.1));
                        ...
                        let fill =
                            bridge::convert_fill_with_outline(&ir_arc.fill, self.outline_color);
                    ...
                    GraphicElement::Text(ir_text) => {
                        let pos = transform
                            .transform(&Point::new(ir_text.position.0, ir_text.position.1));
                        let font_size =
                            ir_text.effects.font.size.1.max(ir_text.effects.font.size.0);
    ...
    fn paint_dnp_cross(&self, layers: &mut LayerSet) {
        let layer = layers
            .get_layer_mut(LayerId::SymbolForeground)
        ...
        let line1 = crate::render_core::graphics::Polyline::from_points(
        ...
        let line2 =
            crate::render_core::graphics::Polyline::from_points(&[top_right, bottom_left], stroke);
    ...
    fn body_bbox(&self) -> BoundingBox {
        ...
        for ge in &self.body_graphics {
            match ge {
                ...
                GraphicElement::Circle(ir_circle) => {
                    let c =
                        transform.transform(&Point::new(ir_circle.center.0, ir_circle.center.1));
    ...
    fn paint_property(
        ...
        let anchor_pos = if let Some((x, y)) = pos {
            ...
            Point::new(
                ...
                self.symbol.position.y + fallback_offset_y,
            )
        };
...
fn compute_arc_from_points(
    ...
    Some(RcArc::new(Point::new(cx, cy), radius, start_angle, end_angle, stroke).with_fill(fill))
}
