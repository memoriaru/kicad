...
use kicad_json5::ir::{
    self, Arc as IrArc, Circle as IrCircle, FillType, GraphicElement, HorizontalAlign,
    Label as IrLabel, PinGraphic as IrPinGraphic, PinShape as IrPinShape, PinType as IrPinType,
    Polyline as IrPolyline, Rectangle as IrRect, Sheet as IrSheet, StrokeType, Symbol,
    SymbolInstance as IrSymbolInstance, VerticalAlign, Wire as IrWire,
};

use crate::constants;
use crate::painter::{
    Junction, Label, LabelShape, LabelType, Mirror, PinGraphic, PinShape, PinType, SheetInstance,
    SheetPainter, SheetPinRender, SheetPropertyRender, SymbolInstance, WirePainter, WireSegment,
};
use crate::render_core::graphics::{Arc, Circle, Fill, Polygon, Polyline, Stroke, StrokeStyle};
...
pub fn convert_symbol(component: &IrSymbolInstance, lib_symbol: Option<&Symbol>) -> SymbolInstance {
    ...
    let (reference_position, reference_rotation, reference_hidden, ref_justify) = ref_prop
        .map(|p| {
            (
                Some((p.position.0, p.position.1)),
                p.position.2,
                p.hide || p.effects.hide,
                &p.effects.justify,
            )
        })
...
fn collect_pins(lib: &Symbol) -> Vec<PinGraphic> {
    let total_graphics =
        lib.graphics.len() + lib.units.iter().map(|u| u.graphics.len()).sum::<usize>();
...
pub fn convert_polyline_with_fill(
    ir: &IrPolyline,
    stroke_color: Color,
...
pub fn convert_rectangle_with_fill(
    ir: &IrRect,
    stroke_color: Color,
    outline_color: Color,
) -> Polygon {
...
pub fn convert_circle_with_fill(
    ir: &IrCircle,
    stroke_color: Color,
    outline_color: Color,
) -> Circle {
