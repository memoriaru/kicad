...
use crate::bridge;
use crate::constants;
use crate::layer::{LayerElement, LayerElementType, LayerId, LayerSet};
use crate::painter::{Junction, Label, LabelType};
use crate::painter::{JunctionPainter, LabelPainter, Painter, SymbolPainter};
use crate::render_core::graphics::{Polygon, Polyline, Stroke};
use crate::render_core::{BoundingBox, Color, Point};
...
impl<'a> SchematicRenderer<'a> {
    ...
    pub fn skip_drawing_sheet(mut self) -> Self {
        self.skip_drawing_sheet = true;
        self
    }
            ...
            if let Some(lib) = self.find_lib_symbol(&comp.lib_id) {
                for ge in &lib.graphics {
                    if let GraphicElement::Pin(p) = ge {
                        ...
                        bbox.expand_point(
                            comp.position.0 + p.position.0 + dx,
                            comp.position.1 + p.position.1 + dy,
                        );
                    }
                }
    ...
    fn render_drawing_sheet(&self, renderer: &mut dyn Renderer) {
        ...
        renderer.draw_polygon(
            &Polygon::from_points(&[(0.0, 0.0), (w, 0.0), (w, h), (0.0, h), (0.0, 0.0)])
        ...
        renderer.draw_polygon(
            &Polygon::from_points(&[(ix1, iy1), (ix2, iy1), (ix2, iy2), (ix1, iy2), (ix1, iy1)])
                .with_stroke(Stroke::new(line_w, border_color)),
        );
        ...
        renderer.draw_polygon(
            &Polygon::from_points(&[
                (ix1 + spacing, iy1 + spacing),
                (ix2 - spacing, iy1 + spacing),
                (ix2 - spacing, iy2 - spacing),
                (ix1 + spacing, iy2 - spacing),
                (ix1 + spacing, iy1 + spacing),
            ])
            .with_stroke(Stroke::new(line_w, border_color)),
        );
        ...
        renderer.draw_text(
            &Point::new(ix2 - 109.0, iy2 - 4.1 + dy),
            "kicad-render",
            font,
            &text_color,
            false,
        ...
        renderer.draw_text(
            &Point::new(ix2 - 109.0, iy2 - 6.9 + dy),
            &format!("Size: {}", self.schematic.metadata.paper.size),
            font,
            &text_color,
            false,
            0.0,
        ...
        if let Some(title) = &tb.title {
            renderer.draw_text(
                &Point::new(ix2 - 109.0, iy2 - 10.7 + dy_title),
                &format!("Title: {}", title),
                title_font,
                &text_color,
                false,
        ...
        if !self.file_name.is_empty() {
            renderer.draw_text(
                &Point::new(ix2 - 109.0, iy2 - 14.3 + dy),
        ...
        renderer.draw_text(
            &Point::new(ix2 - 109.0, iy2 - 17.0 + dy),
            "Sheet: /",
            font,
            &text_color,
            false,
        ...
        if let Some(company) = &tb.company {
            renderer.draw_text(
                &Point::new(ix2 - 109.0, iy2 - 20.0 + dy),
                company,
                font,
                &text_color,
                false,
        ...
        for (comment, cdy) in tb.comments.iter().zip([23.0_f64, 26.0, 29.0, 32.0].iter()) {
            if !comment.is_empty() {
                renderer.draw_text(
                    &Point::new(ix2 - 109.0, iy2 - cdy + dy),
                    comment,
                    font,
                    &text_color,
                    false,
    ...
    pub fn render(&self, renderer: &mut dyn Renderer) {
        ...
        for text_item in &self.schematic.text_items {
            let font_size = text_item
                .effects
                .font
                .size
                .1
                .max(text_item.effects.font.size.0);
            ...
            let color = text_item
                ...
                .font
                .color
                .map_or(default_color, |(r, g, b, _a)| Color::from_rgb(r, g, b));
            ...
            for &(_, is_paragraph) in text_lines.iter().skip(1) {
                let gap = if is_paragraph {
                    paragraph_gap
                } else {
                    interline
                };
