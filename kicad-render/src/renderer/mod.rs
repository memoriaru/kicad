...
use crate::render_core::graphics::{Arc, Bezier, Circle, Polygon, Polyline, Rectangle, Stroke};
...
impl RenderContext {
    ...
    pub fn for_svg(bounds: BoundingBox, scale: f64) -> Self {
        Self {
            bounds,
            scale,
            transform: Matrix::translation(-bounds.x * scale, -bounds.y * scale),
        }
    }
...
pub trait Renderer {
    ...
    #[allow(clippy::too_many_arguments)] // renderer interface, 1:1 port from KiCanvas
    fn draw_text(
        &mut self,
        position: &Point,
        text: &str,
    ...
    fn draw_rect(
        &mut self,
        rect: &Rectangle,
        _fill_opt: Option<Color>,
        _stroke_opt: Option<Stroke>,
    ) {
