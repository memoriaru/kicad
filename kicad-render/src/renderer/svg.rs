...
use crate::constants;
use crate::render_core::graphics::{Arc, Bezier, Circle, Polygon, Polyline, Stroke, StrokeStyle};
    ...
    fn draw_text(
        ...
        let rotation_attr = if rotation.abs() > 0.01 {
            format!(
                ...
                rotation, transformed_pos.x, transformed_pos.y
            )
        } else {
            String::new()
        };
        ...
        if text.contains("^{") || text.contains("_{") || text.contains("~{") {
            ...
            let tspans =
                markup_to_svg_tspans(&parsed, scaled_font_size, transformed_pos.y, &color_str);
...
impl SvgRenderer {
    ...
    fn current_transform(&self) -> Matrix {
        self.transform_stack
            .iter()
            .cloned()
            .fold(Matrix::identity(), |a, b| a.multiply(&b))
    }
    ...
    fn arc_to_svg_path(&self, arc: &Arc) -> String {
        ...
        format!(
            "M {:.2} {:.2} A {:.2} {:.2} 0 {} {} {:.2} {:.2}",
            start.x, start.y, arc.radius, arc.radius, large_arc_flag, sweep_flag, end.x, end.y
        )
    }
}
...
mod tests {
    ...
    fn test_svg_circle() {
        ...
        let circle = Circle::new(Point::new(10.0, 20.0), 5.0).with_fill(Fill::solid(Color::red()));
