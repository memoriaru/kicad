...
pub fn compute_layout(spec: &SymbolSpec) -> LayoutResult {
    ...
    if !left_pins.is_empty()
        && right_pins.is_empty()
        && top_pins.is_empty()
        && bottom_pins.is_empty()
    {
        let half = left_pins.len().div_ceil(2);
...
mod tests {
    ...
    fn test_simple_6pin_ic() {
        ...
        assert_eq!(
            layout.pins.iter().find(|p| p.index == 1).unwrap().side,
            PinSide::Bottom
        );
        assert_eq!(
            layout.pins.iter().find(|p| p.index == 4).unwrap().side,
            PinSide::Top
        );
        assert_eq!(
            layout.pins.iter().find(|p| p.index == 2).unwrap().side,
            PinSide::Left
        );
    }
