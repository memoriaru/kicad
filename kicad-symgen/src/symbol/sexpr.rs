use crate::fmt;
use crate::model::*;
use crate::symbol::graphics::{self, BodyGraphic};
...
fn generate_symbol(spec: &SymbolSpec, version: KicadVersion) -> String {
    ...
    if spec.pins.len() <= 2
        && spec
            .pins
            .iter()
            .all(|p| p.electrical_type == ElectricalType::Passive)
    {
    ...
    let body_graphic = graphics::infer_body_graphic(spec);
    let hw = fmt::BODY_HALF_WIDTH;
    let hh = layout.body_height / 2.0;
    ...
    match body_graphic {
        ...
        BodyGraphic::Triangle => {
            // Op-amp triangle: pointing right, +/- inputs on left, output on right
            s.push_str("      (polyline\n");
            s.push_str("        (pts\n");
            s.push_str(&format!(
                "          (xy {} {})\n          (xy {} {})\n          (xy {} {})\n          (xy {} {})\n",
                fmt::fmt_f(-hw), fmt::fmt_f(hh),
                fmt::fmt_f(hw), fmt::fmt_f(0.0),
                fmt::fmt_f(-hw), fmt::fmt_f(-hh),
                fmt::fmt_f(-hw), fmt::fmt_f(hh),
            ));
            s.push_str("        )\n");
        ...
        BodyGraphic::FunctionBlock { ref sections } => {
            ...
            // Section divider lines + labels across the top
            ...
            for (i, label) in sections.iter().enumerate() {
                ...
                if i > 0 {
                    ...
                    s.push_str("        (stroke (width 0.127) (type default))\n");
                ...
                s.push_str("        (effects (font (size 0.762 0.762))))\n");
            }
        }
    }
    s.push_str("    )\n");
