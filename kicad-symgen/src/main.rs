...
#[derive(Subcommand)]
enum Commands {
    /// Generate a .kicad_sym symbol library file from JSON5 spec
    Symbol {
        /// JSON5 input file
        #[arg(short, long)]
        input: String,
        ...
        #[arg(short, long)]
        output: String,
        /// Library name prefix (default: from spec or "custom")
        #[arg(long)]
...
fn cmd_footprint(
    ...
    let pkg_type = PackageType::from_package_str(package).context(format!(
        "Cannot parse package '{}'. Examples: DIP-8, SOIC-16, SOT-23-5, TSSOP-20",
        package
    ))?;
    let pin_count = extract_pin_count(package).context(format!(
        "No pin count in '{}'. Examples: DIP-8, SOIC-16",
        package
    ))?;
    ...
    let result = templates::generate_from_spec(&spec).context(format!(
...
fn cmd_lib_table(dir: &str) -> Result<()> {
    ...
    if !sym_libs.is_empty() {
        ...
        println!(
            "Generated sym-lib-table ({} libraries) → {}",
            sym_libs.len(),
            out.display()
        );
    ...
    if !fp_libs.is_empty() {
        ...
        std::fs::write(&out, &content)?;
        println!(
            "Generated fp-lib-table ({} libraries) → {}",
            fp_libs.len(),
            out.display()
        );
...
fn default_pitch_for_package(pkg_type: &PackageType) -> f64 {
    match pkg_type {
        PackageType::Dip | PackageType::Sip | PackageType::DipSocket | PackageType::PinHeader => {
            2.54
        }
